...
| `verify_results_fresh.py` | re-runs each producer into `results/`, diffs what appears against what was committed, and restores the committed files afterwards. `--all` widens the set to the producers that need raw traces. A registry key may carry command-line arguments after the script name; the first word is the script. |
