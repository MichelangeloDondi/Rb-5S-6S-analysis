...
| `run_moment_power_map.py` | measures the S0 power of the windowed third, fifth and seventh cumulants across the Lorentzian component, the noise level, the oscilloscope at a named resolution mode, the analysis window and both settings of the model's internal grid, 432 cells on at most eight workers, writing `results/moment_power_map.csv`. `--one-cell` times one before the grid is launched and `--plant` runs the one-worker-against-eight determinism check. Cells are collected in order, so the bytes do not depend on the worker count. |
