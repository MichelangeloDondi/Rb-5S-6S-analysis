...
| `run_sweep_linearity.py` | how linear the frequency axis must be for each observable that reads a line: the rate variation across the analysis window whose forged third cumulant equals the light shift's own, at the archive's configuration, the campaign's tightest licensed waist and its unlicensed 16 micron one, with the bow as a fraction of the actuator's travel, a sub-scan invariance row, and ripples as a percentile over phase. |
...
| `run_cross_arm_ratios.py` | ratios and differences of the windowed second cumulant across the design's axes (power, temperature, window, isotope, hyperfine level), each on two grids and against the model's prediction at 42 um, the retired convention, and 85 um. Writes `cross_arm_ratios.csv` |
