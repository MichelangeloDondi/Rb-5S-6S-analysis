...
| `run_observable_taxonomy.py` | what each family of observables delivers, in the owner's taxonomy: the lineshape alone, the peak centres alone, and the two combined, each on the vapour cell and on a nanofibre, pooled over an inventory of four peaks, five powers, three temperatures, three oscilloscopes and two vertical ranges at six windows. |
| `run_sweep_linearity.py` | how linear the frequency axis must be for each observable that reads a line: the rate variation across the analysis window whose forged third moment equals the light shift's own, at the archive's configuration, the campaign's tightest licensed waist and its unlicensed 16 micron one, with the bow as a fraction of the actuator's travel, a sub-scan invariance row, and ripples as a percentile over phase. |
| `run_transition_ladder.py` | the nS and nD two-photon rungs between 650 and 1050 nm and what each one does to the apparatus. Builds the S and D series by a Rydberg-Ritz fit anchored on the levels this package holds exactly, checks the fit against its own nP ladder, then computes for every rung in band the waist under both input-beam regimes, the Rayleigh range, the collection window's ratio and the axial sign test, the transit width, the Doppler pedestal, both intermediate detunings, the 5S half of the two-photon matrix element as a relative indicator, the ground-state polarizability at the drive, and the thermal occupation at the gap to the nearest nP. Differential polarizabilities are emitted only where a sum over states exists here |
| `run_ruler_tooth_shares.py` | the comb's tooth heights against the phase-modulation law, on the real rulers. A pure phase modulation puts `J_k(2 beta)^2` of the two-photon rate into the tooth at order k, and the committed ruler CSV carries seven heights per trace, so the law is directly checkable and nothing had checked it. It is rejected, by a symmetric excess at the third orders. A free depth does not fix it and a flat pedestal at a few per cent of the comb does, completely. Reports the signal-over-pedestal per order, because that is what decides which teeth a saturation ladder can actually use, and reads a committed CSV, so register A75 applies |
| `run_twin_completeness.py` | the one test of the twin's completeness. An information criterion compares models a fitter can express, and the twin generates seven terms no fit can free, so for those the difference in AIC is undefined and not merely small. This fits nothing and compares what the fits leave behind: wing statistics on real traces against twin traces at matched conditions, under one estimator, on quantities that need no frequency axis. It found the twin's white noise correct for the archive's detrended residual and a per-trace baseline tilt missing from it |
| `run_fringe_rho_recovery.py` | what the fringe contrast can say about the retro power ratio, which the shift channel cannot separate from the differential polarizability. Threads the beam quality, the polarisation and the tilt with the offset it implies, asserts the convolution licence per cell and refuses the cells above it, and reports the derivative that decides whether the channel carries anything at all |
| `run_rf_saturation_ladder.py` | what an EOM depth ladder buys on the fitted waist, and what it costs to skip. A phase modulation holds the total intensity, so every tooth sits at the same light shift and differs only in Rabi frequency, which is the one axis that moves the saturation companion alone. Runs the ladder the archive's own rulers admit, read from `ruler_tooth_shares.csv` and not assumed, and carries the arm with the Lorentzian width frozen because a free one absorbs the companion and measures nothing |
| `run_moment_admission.py` | which windowed moments this archive can actually read. Builds the statistic vector's empirical covariance on the twin's world under the correlation time `noise_model.csv` reports, and admits or refuses each statistic on its measured per-trace signal-to-noise and not on which parity it belongs to. The split comes out exactly by parity: every even order and even ratio clears the floor and every odd one sits two orders of magnitude below it, so the odd ladder is not a weaker channel on this archive but not a channel. Also reports the admitted set's effective rank, which is about three and not its column count, and the covariance's condition number, which is why an interval taken through it needs a stability check before it is quoted |
| `run_kernel_inhomogeneity.py` | whether the composite line may be written as a convolution at each waist, and what assuming so costs. Builds the collected volume as a mixture in which every element carries its own shift, its own transit width and its own homogeneous Lorentzian, the last keyed on the element's own light shift through the saturation companion, and compares it against the same mixture with the kernel held at its weighted mean. The centre channel is immune and the moment channel is not. Each moment is computed on a halved grid too and refused unless the two agree, since a third moment is a small difference of large numbers and the first cut of this producer returned the wrong sign at the widest waist |
...
| `make_model_terms.py` | renders `rb5s6s/model_registry.py` into `results/model_terms.csv` and `docs/methods/model_terms.md`, the registry of which computation path carries each model term |
...
| `run_scenario_forecast.py` | loads every preset under `examples/scenarios/`, refuses any the named scope cannot realise, and forecasts each across its open spans through `forecast_precision`, writing `results/scenario_forecast.csv`. About fifty seconds sequential and twenty at four workers, crc32-seeded per task so the CSV reproduces exactly at any worker count. The scenario layer's end-to-end proof. |
| `run_paired_reference_forecast.py` | forecasts the paired cell-plus-fibre acquisition against unreferenced sweeps, one offset realisation per iteration acquired in both modes, over the spans of the unmeasured lock drift and per-sweep excursion and both detection branches. Writes `results/paired_reference_forecast.csv`, quoted by the fibre scenario of `docs/big_picture/09`. The criterion and limit rows carry their distance to the threshold in their own sigma. The decision row is conditional on the excursion class. Six independent base seeds per configuration, concatenated: 668 to 789 s sequential and 153 to 170 s at six workers on a ten-core machine, measured three times and independently 2026-09-02, crc32-seeded per task and replicate so the CSV reproduces exactly at any worker count. |
| `run_twin_closed_loop.py` | generates dataset_2025 worlds through the public builder, fits them by the record's own joint five-repeat protocol, and stands the recovered medians against the preregistered bands, writing `results/twin_closed_loop.csv` with verdict booleans the prose quotes verbatim. Includes its own gage: a shifted truth must fail. About eight minutes, seed-pinned (its own docstring said four; eight is the figure this table carries until the producer is timed). |
...
| `run_platform_twins.py` | the platform layer: what a cell, a trap, a molasses and a hollow-core mode each deliver, with the interaction length branching on whether the mode is guided and the observable branching on fluorescence against transmission. Carries a `--plant` that checks determinism, reproduces the record's published saturation parameter at the archive's waist by an independent route, and proves the guided branch really ignores the Rayleigh range. Under a second, no traces, writing `results/platform_twins.csv`. |
| `run_digitiser_scale.py` | the vertical scale of every peak, read out of the traces themselves as the modal gap between sorted distinct samples, with the window following from the owner's 5/95 convention. Carries a `--plant` whose four groups were each broken in a clone: a mean instead of a mode, the minimum gap instead of the modal one, the degenerate-trace guard removed and the convention altered, and each fired. Seconds, reads raw traces, writing `results/digitiser_scale.csv`. |
| `run_waist_ladder.py` | the analytic waist ladder: every model term against a known magnification, with the fitted exponents checked against the derivation and the peak-height optimum scanned on a grid, never solved. Under a second, no traces, writing `results/waist_ladder.csv`. |
| `run_moment_power_map.py` | measures the S0 power of the windowed third, fifth and seventh moments across the Lorentzian component, the noise level, the oscilloscope at a named resolution mode, the analysis window and both settings of the model's internal grid, 432 cells on at most eight workers, writing `results/moment_power_map.csv` and its per-rung table `results/moment_power_map_rungs.csv`. `--one-cell` times one before the grid is launched and `--plant` runs the one-worker-against-eight determinism check. Cells are collected in order, so the bytes do not depend on the worker count. |
| `run_moment_power_map_deep.py` | the map's deep configuration, 24 cells at forty thousand traces from the archive's rung up, on at most eight workers, writing `results/moment_power_map_deep.csv` and its per-rung table. About ninety minutes on six workers. |
| `run_three_channel_forecast.py` | the three-channel forecast: one campaign lever at a time from a base point (waist, power, temperature, retro ratio, oscilloscope, lock residual, EOM spacing and depth, window), the coefficient read through the third moment inverted on its noiseless quiet curve, through the fitted centre against power under the repaired lock, and through the two combined with their measured covariance, with the fifth and seventh moments' readmission statistics beside them, writing `results/three_channel_forecast.csv`, whose summed tooth area follows the comb's sum rule only where the transition is far from saturation, which the campaign's own tight waist is not. `--one-cell` times one, `--plant` runs one worker against eight. |
...
| `run_ladder_terms.py` | sizes the three terms that rise together along the temperature arm (the self-broadening signal, the transit's drift, the permeated gas's drift) from committed cells and the package's callables into `ladder_terms.csv`; seconds |
...
| `run_beta_self_theory.py` | the theory self-broadening coefficient into `beta_self_theory.csv`: Zameroski's measured 7S rate converted at his own 393 K, carried to 403.15 K <!-- other-quantity: the vapour cell's own 130 C in kelvin, the temperature the rate is carried to, not twin_realism's radiation_temperature cell --> by the speed average's T^0.3 and scaled to 6S through the ratio of pair-coefficient differences, with the error budget measured term by term. About a second, reads no data |
...
| `run_far_wing_level.py` | the far wing (15 to 40 MHz from the centre) of every canonical condition relative to the fitted peak, its bar from the noise law, the homogeneous width it implies, and its line in N(T) at 225 mW under the three density laws (beta and the Lorentzian floor on the wing channel). Writes `far_wing_level.csv` |
| `run_cross_arm_ratios.py` | ratios and differences of the windowed second moment across the design's axes (power, temperature, window, isotope, hyperfine level), each on two grids and against the model's prediction at 42 um, the retired convention, and 85 um. Writes `cross_arm_ratios.csv` |
...
| `run_prediction_band.py` | Gate A, into `prediction_band.csv`: the predicted coefficient's band from the record's stated inputs against the committed bound, with boolean verdicts the docs quote. It is the first consumer to give `stark_ramp_axial` its collection window from the apparatus instead of leaving it open, and it emits the windows at which the correction changes sign so a later geometry is checked and not assumed to inherit this one |
...
| `annotate_manifest_power.py` | recovers the drive power of the 115 RF-on modulator traces, which `import_data.py` leaves blank because their filenames carry no power. It writes `power_mW_inferred` and `power_inference_basis` and **never touches `power_mW`**, so a logged setting stays distinguishable from a reconstructed one. The temperature rulers take the pinned operating power from their own `_TTTc` name; the `before` rulers take the sweep maximum, the clock in `data_recovered/CLOCK.csv` showing the 225 mW block follows each of them within 2.7 to 5.3 minutes with nothing between. The `after` rulers are left blank: they follow the 25 mW block and no acquisition shows the power being restored, so the clock does not support a value and none is invented. `--check` reports without writing |
| `check_moved_values.py` | greps for every literal a `results/` cell held anywhere in the unpushed range, scoped to files that cite that CSV, with bands graded as bands and the correction record's `now` column graded while its `was` column is not. The complement of `check_references.py`, whose population is the values carrying a `ref:` tag: a plain typed copy is outside it, and eighteen such copies survived a correction wave until this was written. **It cannot see a number that never matched the CSV**, which its own plant demonstrated; that class belongs to the `ref:` tag |
...
the campaign-morning tree. Given a missing tree each prints what it cannot
find. The producers exit 0, so the committed CSV stays the record.
`run_epoch_checks.py` is the exception and writes no CSV at all, so it has
no committed file a zero could defend: it exits 77, the could-not-run code,
which is not a pass. `publish_recovered.py`,
