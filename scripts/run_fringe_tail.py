...
at the waist convention (config.W0_CENTRAL_M) and small-waist config S (16 um)
...
_S0_RECORD = kappa_pred_per_watt(C.W0_CENTRAL_M, C.RHO_RETRO) * 0.225
_S0_SMALL = stark_shift_S0_mhz(0.225, 16e-6, rho=C.RHO_RETRO)
REGIMES = (
    (f"2025 ({C.W0_CENTRAL_M*1e6:.0f}um, {_S0_RECORD:.2f}MHz)",
     C.W0_CENTRAL_M, _S0_RECORD),
...
def main() -> int:
    ...
    print(f"  {'regime':22s} {'rho':>4} {'window':>8} {'d_skew':>16} "
          f"{'d_mu3':>9} {'exc_var':>8} {'f_res':>7} {'wfrac':>7}")
    rows = []
        ...
        for rho in RHOS:
            for wname, tc in WINDOWS:
                ...
                print(f"  {label:22s} {rho:>4.2f} {wname:>8} "
                      ...
                      f"{r['d_mu3']:>+9.4f} {r['excess_var_frac']*100:>7.2f}% "
    ...
    print(f"  {'regime':22s} {'d_skew/f':>10} {'d_mu3/(S0^3 f)':>18} "
          f"{'exc_var(/Var0)/f':>17}")
    for label, w0, s0 in REGIMES:
        ...
        print(f"  {label:22s} {r['d_skew']/f:>10.2f} "
              f"{r['d_mu3']/(s0**3*f):>18.3f} {exc_over_var0/f:>17.2f}")
    ...
    with open(C.RESULTS_DIR / "fringe_tail.csv", "w", newline="") as fh:
        ...
        for r in rows:
            ...
            w.writerow(["d_mu3", k, f"{r['d_mu3']:.4f}",
                        "third-moment change MHz^3 (with - without fringe)"])
            w.writerow(["d_mu3_mc_err", k, f"{r['d_mu3_mc_err']:.4f}",
                        "block-to-block standard error on d_mu3"])
    ...
    # waist" after W0_CENTRAL_M moved to the retired waist convention, and a percentage of the triangle
    ...
    print(f"  variance. At the measured {C.W0_CENTRAL_M*1e6:.0f} um it is negligible")
    ...
    print("  tau_6S window sweep brackets it; the third moment is the stable")
    print("  bracket (config S -0.14 -> -0.16 MHz^3), the standardized skew nearly flat")
    print("  because the same window that grows the moment also grows the")
