...
def main() -> int:
    ...
    # EVERY NUMBER HERE IS READ FROM THE CSV, and the first draft of this paragraph carried
    # five of them as LITERALS -- 0.988, 0.7, 1.0, 0.37 and 5.17. That is the class the rule
    # file names about producers: a literal is invisible to the freshness check, which proves
    # the page matches its generator and never that the generator matches the data, so the
    # sentence would have gone on reading 0.988 after the closure was re-run. It also failed
    # the reference ratchet, which is the cheap instrument that caught it.
    ...
    W("**What this bounds, and what it does not.** The parameter carries the "
    ...
    if sj:
        ...
        W(f"- **C3f. The joint three-session fit puts the light-shift bound "
          ...
          f"variance at fixed code. **A movement under later code was "
          ...
          f"everywhere tested, in the environment of record. This does not "
          ...
          f"predicted envelope, 1.404 to 1.760 in $\\kappa$ over the stated "
    ...
    if pol:
        ...
        W(f"- **Dynamic polarizabilities:** an independent sum-over-states "
          ...
          f"al.'s 1093 but with the **opposite sign** (a blue transition shift). "
          f"**That band is the internal propagation and not the whole "
          f"uncertainty:** it is conditional on this line list and these "
