...
def main() -> int:
    ...
    W("Generated by `scripts/make_results_ledger.py` from the committed "
      ...
      "noted, because they ride on the beam waist $w_0$, which has not been "
      "measured on this bench. Frequencies are transition-axis unless "
      "a `_LASER` note says otherwise.\n")
    W("> **The dominant systematic is the beam waist $w_0$, which has not been "
      "measured on this bench. A direct beam-profile measurement (knife-edge "
      "and/or camera) in a fixed-lock session would measure it, and that "
      "session is proposed rather than scheduled.** At the old 32 µm nominal "
    ...
    if dfr:
        ...
        W("\n> The **$w_0$-band is the largest systematic** (the unmeasured waist, "
          "which the "
