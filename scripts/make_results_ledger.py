...
def main() -> int:
    ...
    W("**What this bounds, and what it does not.** The parameter carries the "
      ...
      "GAUSSIAN WIDTH LEFT OVER once transit is removed at the measured "
    ...
    if ss:
        ...
        W(f"- **C3d. The AC-Stark coefficient from the power lever is an upper bound, "
          ...
          f"and the factor at both saturation arms (2.75 at each), regenerated "
          f"and checked like any number here. **The 2.21 does not, and cannot from "
    ...
    if sj:
        ...
        W(f"- **C3f. The joint three-session fit puts the light-shift bound "
          ...
          f"band. **This sentence called that an exclusion at roughly the "
          f"two-sigma level, and two things qualify it that the sentence did "
          f"not state.** First, the strength is a RANGE and not a number: "
          ...
          f"profile's shape. **And the fuller construction is the robust one**, which no surface said until 2026-08-27: the full-archive fit over both ladders ([`full_dataset_fit.csv`](../results/full_dataset_fit.csv)) puts the limit at $\\kappa \\lt$ 0.944, gives $\\Delta\\chi^2$ of 6.5 to 10.5 across the same envelope, 2.5 to 3.2 $\\sigma$, and ALL FOUR of its leave-one-out arms clear the threshold at 3.17 to 11.48. So the fragility above is a property of the three-session construction and not of the exclusion, and the record's own standing rule is to present the fullest model. **That construction carries a FAILING gate of its own and it is quoted here and not left in the file**: `full_dataset_fit.csv`'s `gate_B4_prior_tension` reads FAIL at 3.78 against a $\\lt$ 3.0 $\\sigma$ criterion, the largest posterior-minus-prior discrepancy across the four peaks, and its preregistration says that above three the fit and the four-point measurement disagree about the same quantity and it is reported and not averaged away. The tension sits on $\\beta_\\mathrm{{self}}$, which is degenerate with the width channel this bound is read from, so nothing here calls that fit robust without naming B4 in the same breath. **And the same criterion applied to THIS construction fails harder, on the same peak, which no surface said until 2026-08-27.** `run_stark_joint` computes no gates, but its own committed `gamma_coll_post` cells against the same beta_self priors give 2.13, 0.82, 4.65 and 0.62 prior-sigmas for 4121, 4154, 4192 and 4207. The three-session fit the headline bound is read from sits at 4.65 on 4192, against the full-archive fit's 3.78 on the same peak. **And 4192 is the common factor in four separately-narrated qualifications**: the prior-tension peak in both fits, the borderline leave-one-out arm in both, the drop that loosens both bounds, and the peak carrying the whole pilot session. gamma_coll and kappa both broaden the line and the only discriminant is the power dependence, so the peak whose collisional width is pulled hardest is the peak whose kappa information is least trustworthy. That is one diagnosis, not four hedges, and the next measurement it names is the per-temperature decomposition the full-archive preregistration already asks for. **The claim that dropping any one peak "
          ...
          f"off at all. **Five wider retractions were attempted in one night "
          f"and all five withdrawn**, listed so that none is revived. The "
          f"$\\chi^2_\\text{{red}}\\approx3.7$ and the coverage study's "
          f"zeroth percentile are both properties of C3d's width-only "
          f"regression and never of this construction, and the second had "
          f"already been withdrawn in-record. The 837-to-1038 construction "
          f"spread is NOT a third-digit effect, and a sentence here called it "
          f"one until 2026-08-27: read like for like at fixed geometry the "
          f"committed spread is 1.231, about 23 per cent, and the tail "
          f"probabilities the two constructions imply differ by about a "
          f"factor of two. What it does not bear on is whether an exclusion "
          f"EXISTS, since both readings put the computed value above the "
          f"limit. (The 1038/837 ratio of 1.24 is not the like-for-like "
          f"figure: 837 is the crossing at central geometry and 1038 is a "
          f"percentile with geometry marginalised, so that ratio mixes a "
          f"construction change with a marginalisation. 1.231 is the "
          f"committed row taken at fixed geometry on both sides.) "
          f"The pass-to-pass factor of two conflates two "
          f"diagnostics, one of them a display-normalisation artefact. And "
          f"the drop-4192 arm alone lands inside the envelope, which is why "
          f"the leave-one-out set above and not that single arm is the "
          f"qualification that holds. The measured waist is the lineage "
          f"measurement, since "
