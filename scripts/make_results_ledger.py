...
def main() -> int:
    ...
    if _uc:
        _alive = _c("noise_where_the_waist_dies", "largest_scale_still_localised")
        W("> **And the degeneracy is demonstrated, not only argued.** The ultra-joint fit "
          "reports an interior minimum in $w_0$, which would be a measurement if the "
          "likelihood carried one. Closed on its own model "
          "([`run_ultra_joint_closure.py`](../scripts/run_ultra_joint_closure.py)), it "
          "carries one only at low noise. Traces were generated by that same forward model "
          "at a known 52 µm, on the archive's own frequency axes, at each trace's own "
          "level, with each condition's own noise law scaled through eight steps from zero "
          f"to one. The waist is localised up to {_alive} times that law and rails above it: "
          f"the interior fraction is {_c('sweep_x0.5', 'interior_fraction')} at half the law, "
          f"{_c('sweep_x0.7', 'interior_fraction')} at seven tenths and "
          f"{_c('sweep_x1', 'interior_fraction')} at the law itself. **So the archive's own "
          "noise is what removes the waist, and halving it would restore the measurement "
          "with no new observable.** Two cautions ride with it: the estimate is biased high "
          "even where it localises, by an amount each rung's own note carries, and the "
          "generator and the estimator share one forward model, so this closes conditioning "
          "and not physics. The synthetic ladder therefore refused the archive "
          f"({_c('real_arm_reached', 'boolean')}), the closure verdict reads "
          f"{_c('closure_verdict', 'waist_recovered')}, and every $w_0$ in "
