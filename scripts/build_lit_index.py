...
PDF_README = ROOT / "PDF_papers" / "README.md"
...
def emit_bib(entries: list[dict]) -> str:
    ...
    for e in sorted(entries, key=lambda x: x["citekey"]):
        note_bits = []
        if e.get("pdf"):
...
def emit_readme(entries: list[dict]) -> str:
    ...
    for e in sorted(entries, key=lambda x: x["citekey"]):
        ...
        rows.append(f"| `{fname}` | `{e['citekey']}` | {held} | "
...
def main() -> int:
    ...
    if args.check:
        bad = False
        for path, new in ((BIB, bib), (PDF_README, readme)):
    ...
    BIB.write_text(bib)
    PDF_README.write_text(readme)
    print(f"wrote {BIB.relative_to(ROOT)} and {PDF_README.relative_to(ROOT)} "
          f"from {len(entries)} lit files")
    return 0
