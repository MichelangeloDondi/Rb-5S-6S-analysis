...
SECTION_ORDER = [
    ...
    ("lan-platforms",    "The host group's platforms: guided fibre, MOT, molasses, lattice"),
    ("unsorted",         "Unsorted"),
]
...
def emit_public_index(entries: list[dict]) -> str:
    ...
    *notes* (docs/lit/), not a shelf of source PDFs. The full holdings table
    ...
    preamble = (
        ...
        "The `loci` column says which thread of the programme a reference "
        "bears on. **P1** is the vapour-cell campaign analysed here. **P2** is "
        "the proposed optical-nanofibre extension, for which no data have been "
        "taken. **THEORY** marks the lineshape derivation, and `M<n>` a "
        "pipeline module.")
    def _row(e):
        rt = ", ".join(e.get("routing") or []) or "—"
        loci = ", ".join(e.get("loci") or []) or "—"
