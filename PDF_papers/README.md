...
| `Nieddu_2019_993nm-5S-6S-two-photon-frequency-reference.pdf` | nieddu2019 | the group's 993 nm frequency reference — same channel, 2019 baseline | **DONE (turn A; published OE copy swapped in 2026-07-14, page 6528/DOI now read off the publisher PDF)**: measured beam **1/e² diam 128 µm ⇒ w₀=64 µm** (f=150 mm) — corroborates the w₀ re-pin; 4 peaks FWHM 2.43–2.60 MHz laser-axis ≈5 MHz transition (corroborates our width); laser MBR-110 @ ~100 kHz. **New apparatus facts (2026-07-14 read) — a CONTRAST with our setup, not a match:** Nieddu's retro = **concave mirror f=75 mm at 2f from the focus** (self-imaging → ρ≈1 by design). **Ours is NOT this** (MD, 2026-07-14) ⇒ the retro-overlap/ρ systematic stays live for the archival data and ρ=1 in the S₀ prediction is an inherited ideal (bounded: S₀ ∝ (1+ρ)/2 → 0.29–0.59 MHz over all ρ; and the strong Doppler-free signal, ∝ρ, argues ρ is not small). Detection = **780 + 795 nm together** through an 800 nm short-pass (R636-10) — unlike our 795-only archival stack, so cross-paper amplitude comparisons must not assume the same collection channel; QWP slots before L1 and CM. |
