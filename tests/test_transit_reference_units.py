...
reader of the public surface would try:
    ...
    transit_fwhm_at_T(130.0, W0_CENTRAL_M)   ->  0.0000435 MHz

The second parameter is a reference transit WIDTH in MHz. `W0_CENTRAL_M` is
a beam WAIST in metres, 4.238e-5 (this record's bore-limited actual focus
since O44/F280, 2026-09-21; at the incident's own date the symbol named the
retired waist convention instead). The function
accepted it and returned a width four orders of magnitude too small, with no
error and no warning. The true value is 1.4459 MHz, reached through the
exported `transit_fwhm_from_w0(w0, T_C)`.
...
transit width, `W0_CENTRAL_M` is the geometry constant a reader meets first
...
from rb5s6s import W0_CENTRAL_M, transit_fwhm_from_w0
from rb5s6s.linefit import transit_fwhm_at_T


@pytest.mark.parametrize("bad", [
    W0_CENTRAL_M,        # the incident: a waist in metres, not a width in MHz
...
def test_the_correct_path_still_gives_the_published_number():
    """1.4459 MHz at 130 C and the bore-limited central waist, as every gate reports.

    RE-PINNED 2026-09-21 (O44/F280): was 0.9575 MHz at the retired waist convention;
    K.W0_CENTRAL_M now reads 42.38 um and the transit width goes as 1/w0, so the smaller waist
    gives the larger width (results/waist_ladder.csv's regenerated row: 1.445930)."""
    assert transit_fwhm_from_w0(W0_CENTRAL_M, 130.0) == pytest.approx(
        1.4459, abs=5e-4)
