...
the archival 50 um waist and material at the October 16 um waist, and both the
...
_OCTOBER = dict(w0_m=16e-6, s0_mhz=5.7)
...
def test_reproduces_reference_direct_mc_at_the_anchor_draw():
    ...
    # d_skew -0.038 (archival) and -0.143 (October).
    a = fringe_tail_mc(**_ARCHIVAL, rho=1.0, seed=7, n_atoms=300_000)
    o = fringe_tail_mc(**_OCTOBER, rho=1.0, seed=7, n_atoms=300_000)
...
def test_mean_pull_preserved_by_the_symmetric_fringe():
    ...
    r = fringe_tail_mc(**_OCTOBER, rho=1.0, n_atoms=300_000, n_blocks=4, seed=1)
...
def test_shorter_coherence_window_resolves_more_fringe():
    # capping the window at tau_6S (< the October transit) leaves more fringe
    # unaveraged -> larger resolved fraction and larger third-cumulant change
    trans = fringe_tail_mc(**_OCTOBER, rho=1.0, n_atoms=300_000, n_blocks=4, seed=1)
    tau6s = fringe_tail_mc(**_OCTOBER, rho=1.0, coherence_s=TAU_6S_S,
...
def test_byte_reproducible_at_fixed_seed():
    a = fringe_tail_mc(**_OCTOBER, rho=1.0, n_atoms=50_000, n_blocks=3, seed=7)
    b = fringe_tail_mc(**_OCTOBER, rho=1.0, n_atoms=50_000, n_blocks=3, seed=7)
