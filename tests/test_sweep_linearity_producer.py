...
CASE_LITERAL = '("archive", K.W0_CENTRAL_M, 0.8208, 6.0, True)'
...
def _first_case_shift(s0: float) -> str:
    ...
    return src.replace(CASE_LITERAL, f'("archive", K.W0_CENTRAL_M, {s0!r}, 6.0, True)', 1)
...
def test_the_shift_literal_check_passes_the_tree_as_it_stands():
    ...
    assert g["CASES"][0][2] == pytest.approx(
        stark.stark_shift_S0_mhz(0.225, K.W0_CENTRAL_M, K.RHO_RETRO), rel=2e-3)
...
def test_a_wrong_shift_literal_is_refused_at_import(factor):
    ...
    pred = stark.stark_shift_S0_mhz(0.225, K.W0_CENTRAL_M, K.RHO_RETRO)
