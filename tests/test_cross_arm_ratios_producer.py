...
def test_every_ratio_carries_its_bar_and_its_three_predictions():
    rows = _rows()
    for q in ("power_ratio_k2", "temperature_difference_k2", "window_ratio_k2",
              "isotope_contrast_k2", "F_contrast_k2_87Rb", "F_contrast_k2_85Rb"):
...
def test_the_isotope_law_is_the_root_of_the_mass_ratio_and_the_transit_widens_85():
    ...
    for r in rows:
        if r["quantity"] == "isotope_contrast_k2" and not r["key"].startswith("pooled"):
...
def test_a_window_seeded_beyond_the_trace_is_refused_not_clamped():
    xa = _producer()
    y = xa.full_profile(xa.MODEL_NU, gamma_coll=0.58, sigma_laser_fwhm=1.6, transit_fwhm=0.93)
    k, _ = xa.k2_checked(xa.MODEL_NU, y, 6.0, 30.0)   # a strip would sit past the grid's edge
    assert not np.isfinite(k)


def test_the_model_k2_scales_as_the_width_squared():
    """A profile stretched by a factor has a windowed k2 that grows, and the
