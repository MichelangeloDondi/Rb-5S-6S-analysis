...
Monte Carlo: at 40 um and 225 mW the third cumulant read 15 per cent off, and the separating variable was the
...
It compared the TRANSVERSE density's k3 ratio, 0.8890, against 1 - 0.1534 = 0.8466, and 0.8466 is F131's
...
Three failure modes, each its own test, and the second is the one that matters:
 ...
 2. AT THE NODE, through the axial mixture at its own z_ratio, the model agrees with the Monte Carlo inside the
    gate's own `ramp_k3_rel` tolerance -- read from the committed `results/kernel_mc.csv`, which is what the
...
NODE = "w40.0_m1.00_r0.940_T130_P225"

#: the Monte Carlo's own saturated-over-weak-field third cumulant at NODE, -0.0032933 / -0.0038461, from the
...
def _cumulants(x, p):
    a = trapezoid(p, x)
    m = trapezoid(x * p, x) / a
    c2 = trapezoid((x - m) ** 2 * p, x) / a
    c3 = trapezoid((x - m) ** 3 * p, x) / a
    return m, c2, c3
...
def test_the_weak_drive_limit_is_the_triangle_to_1e_6_with_its_sign():
    ...
    m, _c2, c3 = _cumulants(X, sat)
    assert abs(m - 2.0 / 3.0) < 1e-6
    # THE SIGN IS PINNED, NOT ABSOLUTED. The first version compared |c3| with |ramp_kappa3|, which a flipped
    # RAMP_SIDE passes -- the one regression class the commit before this one spent itself repairing.
    assert c3 < 0.0, "the blue-sided triangle's third cumulant is negative (RAMP_SIDE, O27)"
    assert abs(c3 - L.ramp_kappa3(1.0)) < 1e-6
...
def test_the_mixture_moments_reproduce_the_axial_closed_form():
    ...
    for zr in (0.26, 0.667):
        ...
        assert abs(mm["k3"] - ma["skew_standardized"] * ma["var"] ** 1.5) < 1e-5
    mm0 = L.ramp_mixture_moments(1.0, 0.0, X, weak)
    assert abs(mm0["mean"] - 2.0 / 3.0) < 1e-4 and abs(abs(mm0["k3"]) - 1.0 / 135.0) < 1e-5
...
def test_the_band_node_agrees_with_its_monte_carlo_through_the_mixture():
    ...
    model agree inside the kernel gate's own `ramp_k3_rel` tolerance. Read from the committed table."""
    from rb5s6s.kernel_gate import READINGS

    row = None
    with (ROOT / "results" / "kernel_mc.csv").open() as fh:
        for r in csv.DictReader(fh):
            if r["node"] == NODE and r["quantity"] == "ramp_k3_rel":
                row = r
    assert row is not None, f"{NODE}'s ramp_k3_rel row is absent from results/kernel_mc.csv"
    mc, model = float(row["mc"]), float(row["model"])
    assert mc < 0.0 and model < 0.0, "both are blue-sided third cumulants and negative (O27)"
    dev = abs(mc - model) / abs(model)
    assert dev < READINGS["ramp_k3_rel"], (
        f"the node's Monte Carlo and model differ by {dev:.4f}, over the gate's "
        f"{READINGS['ramp_k3_rel']}: the weak-field reference read 0.153 here before F133's term")
...
def test_the_transverse_factorisation_stays_inside_its_stated_error():
    ...
    _m, _c2, c3w = _cumulants(X, weak)
    _m, _c2, c3s = _cumulants(X, sat)
    ratio = c3s / c3w
    ...
    _m, _c2, c3x = _cumulants(X, slipped)
    assert abs(c3x / c3w - MC_SAT_OVER_WEAK) / MC_SAT_OVER_WEAK > FACTORISATION_ERR, (
