...
| `test_no_shadowed_script_names.py` | No tracked executable outside `scripts/` may wear a committed script's basename. Written after a scratchpad `port_to_mirror.sh` shadowed the committed one, copied `.github/`, and disabled the public repository's CI trigger for the second time in three days. The name is the expensive part: an outside reader read the committed tool, found it correct, and cleared the port. Its blind region is stated in the file, and it is large. |
