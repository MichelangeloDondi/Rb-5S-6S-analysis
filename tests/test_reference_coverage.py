...
RE-SEEDED 2026-09-13: `docs/LITERATURE_INDEX.md` fell 163 to 158 because the index was regenerated from notes whose summaries now carry parenthetical uncertainties and no bare decimals (achar2025, lahad2019, nasoni2026), a genuine fall.
RE-SEEDED 2026-09-18: `docs/wiki/self-broadening.md` fell 78 to 71 and `docs/methods/03_the_ac_stark_ramp.md` rose 69 to 72. The wiki fell because every 5S-6S width computed from Zameroski's 5S-5D coefficients left with F138's retraction (the C1 board's twin seat), keeping the abundances, the pressures, the ordering and the sign reversal. The methods page rose by three: the saturated density's stated error against the exact closed form (1.9 and 1.3 per cent) and the transit's share of the coherence rate at 40 um, each a derived quantity of the paragraph it sits in rather than a cell of any table, with its finding named in the text.
RE-SEEDED 2026-09-24: `docs/UNCERTAINTY.md` fell 33 to 28 because section 3a's window-drift paragraph was rewritten against its cells at the calculated waist (F506): its counts became bound references and the retired waist's five typed decimals left with the sentence that carried them.
RE-SEEDED 2026-09-25: four files fell and one rose under the owner's order of 2026-09-24 that every uncertainty print at two significant digits. The wavemeter's settled floor became a bound pair in `docs/APPARATUS.md` (160 to 158), `docs/plan/01_aim-and-failure-modes.md` (26 to 24) and `results/README.md` (116 to 114), and the magic wavelengths with their band edges became bound references in `docs/big_picture/01_why-this-line.md` (65 to 61). `docs/notes/two_photon_saturation_companion.md` rose 172 to 178: three subset fits that no committed table holds, written in kHz as +180 +/- 120, -152 +/- 30 and -287 +/- 197, are written in MHz so that each uncertainty carries two significant digits, which turns six integers into six decimals. Ten paid, six raised, and the raise is the rule's own arithmetic.
RE-SEEDED 2026-09-25, the landing of the fold: six files fell and none rose. The magic wavelengths 1287.9 and 1339.6 nm were bound to `polarizability.csv` wherever the renderer had already bound one copy (`README.md` 13 to 12, `docs/CLAIMS.md` 91 to 90, `docs/LITERATURE.md` 132 to 131, `docs/big_picture/01` 61 to 55, `docs/FUTURE_TRANSITIONS_titsapph.md` 360 to 353 with C6(5S+6S) bound to `beta_self_theory.csv`), and the wavemeter's settled floor in `docs/APPARATUS.md` (158 to 157) became a bound value.
...
RE-SEEDED 2026-09-25, the fold's repair pass: six files fell as the moved magic wavelength and the sharing numbers were bound to their cells (README 12 to 11, CLAIMS 90 to 85, FUTURE_TRANSITIONS 353 to 351, big_picture/01 55 to 47 and two more), and four rose by caveats whose figures come from a test, a finding or the arithmetic of a bound cell: UNCERTAINTY 28 to 30 (F521's 1.6 to 1.9), big_picture/04 39 to 40 and plan/00 47 to 48 (the bore factor's 9.1 per cent, a strict xfail's reading), model_selection_prereg 22 to 23 (the AIC arithmetic on the bound 63.3). The total falls.
RE-SEEDED 2026-09-25, the floor's repair: two files fell and none rose. The numbers of the polarizability bullet in `docs/RESULTS.md` that a committed cell holds (the tune-out reproduction, the static alpha_5S and the three magic wavelengths, one written twice) were bound to `polarizability.csv` (309 to 303), and the guided-mode design note's crossings and its alpha_6S at 1064 nm were bound to the same cells at the cells' own decimals (`docs/notes/guided_mode_two_photon_design.md`, 314 to 306).
"""
