...
| `coverage_grid.csv` | `run_coverage_grid.py` | Leg 2 of the twin validation: how often the fitter's 95 per cent width intervals cover the injected truth when the truth carries a defect the fitter lacks, a thousand law-weighted trials per configuration, defect shape by size. The function a referee can read: the intervals stay conservative under the convolved ramp and the population channels, and collapse under an unmodelled Lorentzian wing component, 0.450 at a tenth of a megahertz and zero past three tenths. Nominal covers at 0.994, the error convention's own deliberate inflation, so the preregistered 0.95 band ships failed with that mechanism named. The halved-transit plant zeroes coverage, so the leg can fail. |
