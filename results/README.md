...
| `sobol_acquisition.csv` | `run_sobol_acquisition.py` | Sobol first-order and total-effect indices of the width precision over the five acquisition knobs, with bootstrap errors; the floor exponent and tau range are read from `noise_model.csv` at run time. |
