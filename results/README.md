...
| `scenario_forecast.csv` | `run_scenario_forecast.py` | Each shipped scenario preset forecast through the public path at the three points of its open waist span, with the fitter matched to the injected ramp and with the ramp deliberately omitted, so the cost of ignoring the asymmetric term is a measured column: at the proposed 16 um focus the width error doubles, at 8 um it grows ninefold. The nanofibre preset adds its solved-mode rows. Per-row status, spans carried in the notes. |
