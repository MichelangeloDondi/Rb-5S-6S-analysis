...
| `cumulant_window_check.csv` | `run_cumulant_window_check.py` | What a self-centred windowed third cumulant keeps of the ramp's own $S_0^3/135$, for this record's line, kernel conventions named in every note: the `survival` rows (0.53 to 0.44 full line, 0.58 to 0.49 bare core at the 3 MHz reference, the transit at the archive's 130 C since 2026-09-04), the `survival_vs_S0` rows (shift-independent to first order, the window clipping more of the composite's tails as the shift grows, once the ramp is resolved on the model's grid, and each row prints the grid cells its ramp spans), and the mean pull's windowed excess of [0.051](cumulant_window_check.csv "ref:cumulant_window_check:kappa1_window_excess_pct:gc0.55") per cent, every row checked at half the model grid step, half the ambient grid, twice the window grid and twice the fixed-point passes, against the unrounded value. The rows every prose statement of the moment hierarchy quotes ([methods, the parameter-free moment hierarchy](../docs/methods/03_the_ac_stark_ramp.md#the-parameter-free-moment-hierarchy-the-form-test)). |
| `twin_term_census.csv` | `make_twin_term_census.py` | The twin's terms censused from the live code: for each model term, whether the public forecast path carries it, whether the example's world does, which module owns the physics, and what the fitter does with it, with a provenance note per row. Signatures and layer keys are inspected, never recalled; a deliberate absence says so and cites its clause. |
| `sobol_acquisition.csv` | `run_sobol_acquisition.py` | Exact Sobol first-order and total-effect indices of the width precision over the five acquisition knobs; errors are the floor exponent's fitted se propagated through the closed form, the Saltelli/Jansen run kept as a cross-check, and the floor exponent and tau range read from `noise_model.csv` at run time. |
| `scenario_forecast.csv` | `run_scenario_forecast.py` | Each shipped scenario preset forecast through the public path at the three points of its open waist span, with the fitter matched to the injected ramp and with the ramp deliberately omitted, so the cost of ignoring the asymmetric term is a measured column: at the proposed 16 um focus the width error rises about three quarters, and at 8 um the two presets that share the cell half disagree by a factor of two, 5.6 against 11.5, so that rung is a diagnostic, not a forecast. At the archive's own 62 to 68 um the two columns now agree to a part in a million, where the retired transit made the mismatched fitter under-report its error by 13 per cent. The nanofibre preset adds its solved-mode rows. Per-row status, spans carried in the notes. |
| `paired_reference_forecast.csv` | `run_paired_reference_forecast.py` | Error forecast of the paired cell-plus-fibre acquisition against the unreferenced 2025 default: each iteration's one offset realisation is acquired in both modes at one depth, the joint fit shares the offset only, and each criterion and limit row carries its distance to the threshold in its own sigma, and the criterion rows say unresolved when under one. The criterion fails at the cleanest spanned class, is unresolved at the comb-limit class at the run's own error, and clears from the mid class, so the decision row is conditional on the scheduled lock characterisation. Sensitivity rows carry a baseline and nine design constants doubled and halved. |
| `twin_closed_loop.csv` | `run_twin_closed_loop.py` | Leg 1 of the twin validation, bands preregistered before the first run: worlds at the 2025 dataset's own conditions, fitted by the record's own five-repeat protocol. The recovered widths land in band (A1, A2 True) and the leg can fail (G1 True: a shifted truth breaches the band). A3 is False and committed as found: the reported error reads the noise law's absolute floor against normalised units, a unit seam the note diagnoses, with the unit-faithful rerun queued behind the law loader. |
| `quantisation_crosscheck.csv` | `run_quantisation_crosscheck.py` | Leg 4 of the twin validation: the range each condition's measured lattice step implies at the manual's twelve high-resolution bits, against the scope's settable ranges. Fourteen of twenty sit on the coarse ladder; six cluster near 1.4 times a settable value, consistent with the manual's calibrated Fine adjustment or a root-two sigma factor, a bench-distinguishable pair the D1 row carries as found. The gage row proves the check can fail. |
| `coverage_grid.csv` | `run_coverage_grid.py` | Leg 2 of the twin validation: how often the fitter's 95 per cent width intervals cover the injected truth when the truth carries a defect the fitter lacks, a thousand law-weighted trials per configuration, defect shape by size. The function a referee can read: coverage collapses under an unmodelled Lorentzian wing component, reaching zero at and beyond the component sizes `kernel_k3.csv` measures in the data, while the convolved-ramp and population channels stay near nominal except at the largest omitted ramp, which now under-covers. The nominal row sits above the preregistered 0.95 on the error convention's own deliberate inflation, so that band ships failed with the mechanism named. Every coverage figure lives in the file itself and none is restated here. The halved-transit plant zeroes coverage, so the leg can fail. |
| `moment_power_map.csv` | `run_moment_power_map.py` | The S0 power each windowed odd cumulant carries, measured on the twin across the Lorentzian component, the noise level, the oscilloscope at a named resolution mode, the analysis window and both settings of the model's internal grid: a log-log slope over an S0 ladder spanning the archive's shift and the campaign's, with the standard error and the worst per-rung signal-to-scatter the fit rested on. The `min_snr_over_rungs` column moved definition with the merged producer: it is now the median over the median's own standard error, 0.8 of the mean's, which the first map's file carried under the same name. The noiseless limit with the layers off returns the cubic law **only with `resolve_shift` on** (`tests/test_moment_power_map_producer.py` pins it, since no noiseless arm is in this file); with the shipped default it returns 2.82, because the model sizes its internal convolution grid by the narrowest kernel and not by the shift, so a shift well below the kernel widths falls inside one cell. That is why the setting is an axis of this file and not a footnote: in the noiseless limit with the physics layers off, the unresolved third cumulant reads 68 per cent high at a 0.18 MHz shift, a few per cent off at the archive's 0.364 with a sign that depends on the window, and equal to the resolved one at 1.0 MHz and above; under the layers and the archive's noise the two settings agree within their own scatter at every rung, so the file's two arms measure the cost of the choice only where the noise allows, so the campaign's own regime is untouched and the archive's is not. **The `min_snr_over_rungs` column is read before any slope is quoted, and it is not a signal-to-scatter.** It is the smallest, over the five rungs, of the median cumulant's magnitude divided by the median's own standard error. Where the true cumulant lies below the noise the median's magnitude is itself inflated by that noise, so the column carries the inflation it was meant to police: its largest value on the grid belongs to a rung whose noiseless value stands below the median's own standard error (`snr_true` in the per-rung table), and per trace the signal is a hundredth of the scatter. A slope fitted through such rungs reports noise wearing a power law. The per-rung table beside it, `moment_power_map_rungs.csv`, carries the fraction of traces returning a negative cumulant, which is the statistic that cannot be inflated. |
| `moment_power_map_rungs.csv` | `run_moment_power_map.py` | The same map, one row per rung: the median cumulant of each order, its standard error as a median, and the fraction of traces returning a negative value, which is the statistic that cannot be inflated by noise. A fraction near one half is no measurement whatever the median says. |
| `moment_power_map_deep.csv` | `run_moment_power_map_deep.py` | The deep configuration, 24 cells at forty thousand traces on a ladder from the archive's 0.364 MHz up, through the package estimator: the fitted power of the third cumulant has median 2.97 over the 24 cells (spread 2.3 to 5.4) and median 3.06 over the four kernel settings at a 6 MHz half-window, one of which carries no Lorentzian component at all, each row with its own error. Re-derive: `python -c "import csv,statistics as s; r=[x for x in csv.DictReader(open('results/moment_power_map_deep.csv')) if x['order']=='3']; print(s.median(float(x['s0_power']) for x in r), s.median(float(x['s0_power']) for x in r if x['window_mhz']=='6'))"`. |
| `moment_power_map_deep_rungs.csv` | `run_moment_power_map_deep.py` | Its per-rung table: the median cumulant of each order, its standard error as a median, the fraction negative, the same estimator's noiseless value (`k_quiet`), the fraction of traces with the wrong sign against it, the quiet value over the standard error, and the trace count at which the median would stand three standard errors from zero. The status is DIAGNOSTIC where the wrong-sign fraction is below 0.35. At the archive's 0.364 MHz the third cumulant's sign is settled in no cell and the count to three sigma has median 9.1e+04 traces; at 1 MHz 2 of 24 cells and 2.1e+02; at 2 MHz 22 of 24 cells and 3.4. The fifth cumulant, whose true sign is negative, settles in every cell at 2 MHz and the seventh in 16 of 24 cells, and neither adds a channel, since their windowed forms do not converge on the ramp's own cumulants as the window widens and their ratios to the third carry no power dependence. |
...
| `stark_sweep.csv` | `run_stark_sweep.py` (M4e, C3d) | the AC-Stark coefficient BOUND from the power lever: one shared κ (S₀=κP) fit to the 130 °C FWHM-vs-power (from `power_sweep.csv`). **`S0_225mW_ub95_profile`** is a 95% profile-likelihood upper bound of 0.63 MHz on S₀ at 225 mW, above `S0_225mW_pred` (0.364); replaced as the headline by `stark_joint.csv` (three sessions, < 0.26 MHz) and kept as the independent width-only bracket. (The Wald `S0_225mW_ub95`=3.1 / `_ub95_raw`=1.5 are replaced diagnostics: the fit rails at κ=0, where a linearized error has no coverage.) A BOUND, not a measurement: the shift is dead in the 2025 drift, so only the ramp's width broadening (∝S₀²) constrains κ, and `chi2_red`≈4 (block-to-block width scatter) is folded into the bound. a fixed lock would measure the pull ∝S₀ directly. |
...
| `power_time_sign_test.csv` | `run_power_time_sign_test.py` (needs the excluded trees) | the power-versus-time sign test on the rehearsal's bidirectional ladders: per-peak slopes of a model-independent width against power and against time, the rehearsal ladder directions from its trigger stamps (the pilot arm has none and uses its documented run order), and the pooled two-model comparison. Every row DIAGNOSTIC, a reanalysis of held sessions against a design question, and the deliberately biased widths never meet the fitted ones |
...
| `prediction_band.csv` | `run_prediction_band.py` | Gate A. Whether the predicted Stark coefficient's own band reaches the record's bound, carrying the waist band squared, the retro-ratio error, the polarizability spread against Orson 2021 and a power calibration spanned because no one holds it. The verdicts are booleans so the comparison is never re-made by eye at a different number of digits. It also closes the fluorescence collection window that `stark_ramp_axial` has carried open unset: the window biases a pure-ramp fit low, so correcting it raises the bound, and the rows carry the two windows past which that correction stops being conservative. |
...
decompositions sit below this file's own prediction cell (`S0_225mW_pred`, which lags the
sweep's by 0.016 MHz, the known lag recorded above) and the drop-4192 subset's
bound sits above both predictions, by slightly more than the lag against this
file's cell and by two thousandths of a megahertz against the sweep's. That margin is
measured against the sweep's prediction cell, an ENVELOPE row, and it is two
thousandths of a megahertz against the span from [0.316](stark_sweep.csv "ref:stark_sweep:S0_225mW_pred_lo:shared") to [0.396](stark_sweep.csv "ref:stark_sweep:S0_225mW_pred_hi:shared") MHz
between the sweep's sensitivity anchors, which it refuses to call band edges, so
it decides nothing: a distance from a sensitivity scan is not a sigma, as the
sweep's own note says. `lopo_dchi2_pred` was read as showing no single peak driving the result. That reading is retired: positivity was never the test, the threshold is 2.706, and no count of arms is quotable because the arms are separate likelihoods. The committed note text was brought into line with the corrected producer by hand on 2026-08-27 (20a0a829), since the producer needs five hours and an excluded tree. The numeric cells are untouched. The
