...
__all__ = ["ClippedBeam", "mode_weights_for_m2", "axial_half_intensity_depth_m",
          "effective_transit_radius", "clipped_transit_fwhm_mhz"]
...
class ClippedBeam:
    ...
    def b_table_limit_m(self, z_m: ArrayLike) -> np.ndarray:
        """The largest impact parameter `u(b, z)` will accept at this z without raising:
        `xi_max * _r_scale(z)`, in metres. `_r_scale` is a "generous, purely NUMERICAL (not
        physical)" reference (its own docstring) built from the UNCLIPPED comparison Gaussian, and
        it is NOT guaranteed to exceed this beam's own true `w(z)` -- measured on
        `ClippedBeam.at_focus(W0_CENTRAL_M)`, `w(0)/_r_scale(0) = 1.34`: the real, aperture-clipped
        focus is a third WIDER than its idealised unclipped reference, because `at_focus` solves
        for the input radius that gives the TRUE beam a target focus, and the reference beam built
        from that same input radius is a different (unclipped) beam that need not share it. A
        sampler drawing impact parameters in units of `w(z)` (`volume_line.sample_atoms`) must cap
        its draw at THIS, the table's own reach, not at a multiple of `w(z)` (b_merge task,
        disagreement 2)."""
        z = np.asarray(z_m, dtype=float)
        return self.xi_max * np.abs(np.asarray(self._r_scale(z), dtype=float))

    def u_fast(self, b_m: ArrayLike, z_m: ArrayLike) -> np.ndarray:
        """`u` for the atom Monte Carlo's chords (`volume_line.joint_spectrum(chord="beam")`), which read
        the beam at every point of every atom's path: the same checked spline, with one convention changed.
        A radius beyond the table's `xi_max` returns 0, the tabulated extent of the beam, never the edge
        value `u(clamp=True)` gives, which along a chord would be a constant tail of light. A `z_m` outside
        the tabulated window raises, as `u` does."""
        b = np.asarray(b_m, dtype=float)
        z = np.asarray(z_m, dtype=float)
        bb, zz = np.broadcast_arrays(b, z)
        shape = bb.shape
        zf = self._bounds_check_z(zz.reshape(-1), clamp=False)
        xi = np.abs(bb.reshape(-1)) / self._r_scale(zf)
        out = np.zeros(zf.shape)
        inside = xi <= self.xi_max
        if np.any(inside):
            out[inside] = np.maximum(self._spline.ev(zf[inside], xi[inside]), 0.0) / self._i_peak
        return out.reshape(shape)

    @classmethod
    def at_focus(cls, w_act_m: float, *, m2: float = 1.0, d_ap_m: float = 0.0, rtol: float = 1e-5,
                 **kw) -> "ClippedBeam":
        """The clipped beam whose actual (same-reading) focus `actual_focus_m()` is `w_act_m`: the
        input radius found by a root-find on the construction itself, about twenty builds of a
        fifth of a second each. A twin world built at a waist has to be the beam that READS that
        waist, and no closed form maps a reading back to an input radius (`lineshape.
        aperture_onaxis_factor_actual`'s docstring). The actual focus falls as the input widens,
        down to the bore's floor, so a `w_act_m` below the floor raises with the floor's value, and
        one wider than a 0.2 mm input gives raises too."""
        w_act_m = float(w_act_m)

        def focus(w_in: float) -> float:
            return cls(w_in_m=float(w_in), m2=m2, d_ap_m=d_ap_m, **kw).actual_focus_m()

        lo, hi = 2e-4, 3e-2
        f_lo, f_hi = focus(lo), focus(hi)
        if not (f_hi < w_act_m < f_lo):
            raise ValueError(
                f"ClippedBeam.at_focus: {w_act_m * 1e6:.3f} um is outside what this bore and lens "
                f"make ({f_hi * 1e6:.3f} um at a {hi * 1e3:.0f} mm input, the floor, to "
                f"{f_lo * 1e6:.3f} um at {lo * 1e3:.1f} mm)")
        w_in = brentq(lambda x: focus(x) - w_act_m, lo, hi, rtol=rtol)
        beam = cls(w_in_m=float(w_in), m2=m2, d_ap_m=d_ap_m, **kw)
        if abs(beam.actual_focus_m() - w_act_m) > 10.0 * rtol * w_act_m + 1e-12:
            raise ValueError(
                f"ClippedBeam.at_focus: the root-find returned a focus of {beam.actual_focus_m() * 1e6:.4f} um "
                f"for {w_act_m * 1e6:.4f} um asked")
        return beam
...
def shift_moments(beam, half_window_m: float, n_photon: int = 2,
    ...
    for zi in z:
        ...
        wt = (np.asarray(rate(u), float) if rate is not None else u ** int(n_photon)) * r
        rows.append((r, u, wt))
        n += float(trapezoid(wt, r)); s1 += float(trapezoid(wt * u, r))
    m = s1 / n
    var = mu3 = 0.0
    for r, u, wt in rows:
        var += float(trapezoid(wt * (u - m) ** 2, r))
        mu3 += float(trapezoid(wt * (u - m) ** 3, r))
    return {"mean": m, "var": var / n, "mu3": mu3 / n}
...
class _FreeGaussianBeam:
    """The free-space diverging Gaussian `fullmodel.transit_collection_factor` already assumes
    in closed form, wrapped so `effective_transit_radius` can read it the same way it reads a
    `ClippedBeam`. `clamp` is accepted and ignored: an unclamped analytic Gaussian has no table
    edge to clamp to. Private: `clipped_transit_fwhm_mhz` below builds and discards one, and no
    other caller needs it."""

    def __init__(self, w0_m: float, m2: float):
        self.w0_m, self.m2 = float(w0_m), float(m2)
        self.z_R_m = math.pi * self.w0_m ** 2 / (self.m2 * LAMBDA_LASER_M)

    def w(self, z_m, clamp: bool = False):
        return self.w0_m * np.sqrt(1.0 + (np.asarray(z_m, dtype=float) / self.z_R_m) ** 2)

    def u(self, b_m, z_m, clamp: bool = False):
        w = self.w(z_m)
        return (self.w0_m / w) ** 2 * np.exp(-2.0 * np.asarray(b_m, dtype=float) ** 2 / w ** 2)


def clipped_transit_fwhm_mhz(w0_m: float, m2: float, T_C: float, *, isotope: int = 87,
                             beam=None, z_ratio: Optional[float] = None,
                             half_window_m: Optional[float] = None, rate=None) -> float:
    """The collected transit FWHM (MHz, transition axis), F374's substitution carried as the one
    function both sides call (F471, plan item V5.19a): `scripts/run_kernel_mc.py`'s Monte Carlo
    reference and `scripts/run_ultra_joint.py`'s Cell, so a validated node certifies the transit
    the fitter actually runs rather than a Gaussian-only placeholder.

    Returns the closed-form bare transit (`constants.transit_fwhm_from_w0`) times
    `fullmodel.transit_collection_factor`'s free-space collection factor, times the ratio of
    `effective_transit_radius` on a free-space Gaussian over `beam`. `beam=None` leaves that
    ratio at one, so a caller who never names a beam (or passes its own free-space Gaussian
    object) gets exactly the old Gaussian-only number back, unchanged to the bit.

    `rate` is the chord signal's weight as a function of the on-axis field factor (array in,
    array out). The default `None` reduces `effective_transit_radius` to `u**2`, the two-photon
    unsaturated slice law `transit_collection_factor` itself already uses, so a caller that
    leaves it out stays on that same convention instead of picking up an undeclared saturated
    weighting. `scripts/run_kernel_mc.py` passes its own saturated rate; `scripts/run_ultra_joint.py`'s
    Cell does not, since it has no ready route to that table, and the resulting gap against the
    Monte Carlo's own reading is measured and reported rather than assumed away.

    `z_ratio` and `half_window_m` let a caller that already has them skip recomputing
    `fullmodel.collection_z_ratio_m2` and the physical half-window; a caller with neither gets
    both derived the way the model's own callers already do.
    """
    from .constants import transit_fwhm_from_w0
    from .fullmodel import collection_z_ratio_m2, transit_collection_factor
    zr = collection_z_ratio_m2(float(w0_m), float(m2)) if z_ratio is None else float(z_ratio)
    ref_bare = transit_fwhm_from_w0(float(w0_m), float(T_C), isotope=isotope)
    if zr <= 0.0:
        return ref_bare
    corr = 1.0
    if beam is not None and not isinstance(beam, _FreeGaussianBeam):
        hw = half_window_m
        if hw is None:
            z_R = math.pi * float(w0_m) ** 2 / (float(m2) * LAMBDA_LASER_M)
            hw = zr * z_R
        ref_beam = _FreeGaussianBeam(float(w0_m), float(m2))
        corr = (effective_transit_radius(ref_beam, hw, rate=rate)
               / effective_transit_radius(beam, hw, rate=rate))
    # grouped as ref_bare * (collection_factor * corr), matching the multiplication order the two
    # call sites used before this function existed, so a caller comparing against an old number by
    # exact equality is not tripped by a last-bit rounding difference from a different grouping
    return ref_bare * (transit_collection_factor(float(w0_m), float(m2)) * corr)
