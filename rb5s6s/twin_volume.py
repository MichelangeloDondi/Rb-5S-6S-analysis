...
__all__ = ["stark_s0_mhz", "synthetic_traces"]
...
def synthetic_traces(*, beam, T_C: float, S0_mhz: float, gamma_hom_mhz: float,
    ...
    never the fitter's `volume_line.JointTable` (module docstring).
    ...
    this `beam`'s waist into `S0_mhz`, through the corrected on-axis factor, for a caller who has
    a power rather than an S0). The world line is computed ONCE (cached, see
    `_world_line_cached`), peak-normalised (`shape = world / world.max()`), then given the same
    ...
    `clean = a*shape + base`. `centre_mhz` shifts the line: it is evaluated at `nu - centre_mhz`,
    so its own peak lands at `nu = centre_mhz`, matching `forecast.synthetic_traces` exactly.
    ...
    this module's own docstring for what is out of scope here).
    ...
    peak = world.max()
    if peak <= 0.0:
        raise ValueError("twin_volume.synthetic_traces: the joint line vanished on this axis: "
