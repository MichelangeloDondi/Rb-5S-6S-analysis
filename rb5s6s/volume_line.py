...
measures the cost: the third moment wrong by about a factor of two at the bench's 42.38 um
...
def sample_atoms(rng: np.random.Generator, n: int, beam, T_C: float, half_window_m: float, *,
                 isotope: int = 87, b_cut: float = B_CUT, return_geometry: bool = False,
                 ) -> Tuple[np.ndarray, ...]:
    ...
    speed (m/s), the peak intensity factor, and the (unnormalised) crossing-flux weight. With
    `return_geometry=True` it also returns each atom's impact parameter b (m) and axial position
    z (m), the two numbers a beam that is not Gaussian needs to be read along the chord
    (`joint_spectrum(chord="beam")`). The draws are the same, so the first four arrays are too.
    """
    n = int(n)
    ...
    r_max = cut * w
    # A TABLE-BACKED BEAM'S OWN REACH CAPS THE DRAW (b_merge task, disagreement 2). `w` here is
    # `beam.w(z0)`, the beam's TRUE 1/e^2 width; a table-backed beam's `u()` validates a query
    # against a DIFFERENT, purely numerical reference scale (`ClippedBeam._r_scale`, documented as
    # "generous" but not guaranteed to exceed the true width -- measured 34 per cent short of it at
    # the focus of `ClippedBeam.at_focus(W0_CENTRAL_M)`), so `cut * w` alone is not bounded by what
    # the table can return. `b_table_limit_m(z0)`, where the beam offers it, is exactly that
    # table's own reach; capping there means `u(b, z0)` below never raises for this draw, by
    # construction, whatever `w(z0)` and its reference scale's ratio happen to be. A GaussianBeam
    # has no such method and this line does not touch it (`getattr` default), so its bit-identical
    # random stream (the comment above) is unaffected.
    if not gaussian:
        table_limit = getattr(beam, "b_table_limit_m", None)
        if table_limit is not None:
            r_max = np.minimum(r_max, np.asarray(table_limit(z0), dtype=float))
...
def joint_spectrum(*, S0_mhz: float, gamma_hom_mhz: float, beam, T_C: float,
                   ...
                   homog_margin_mhz: float = HOMOG_MARGIN_MHZ, chord: str = "auto") -> np.ndarray:
    ...
    THE CHORD (C6b, 2026-09-22). `chord="gaussian"` writes each atom's intensity along its path as
    u_b exp(-2 tau^2), which is EXACT for a Gaussian beam and only for one: a bore-clipped focus
    carries rings, so an atom crossing it sees an intensity history that is not a Gaussian in time,
    and both its transit and its accumulated light shift follow that history. `chord="beam"` reads
    the beam itself at every point of the path, u(sqrt(b^2 + (w tau)^2), z), through the beam's
    `u_fast` when it has one (`beam_field.ClippedBeam`) and its `u` otherwise, and the phase is the
    running integral of that intensity. At a `GaussianBeam` the two agree to floating point, which
    is this option's plant (tests/test_volume_line.py). `"auto"` takes "gaussian" for a
    `GaussianBeam` and "beam" for anything else, so a clipped beam is never read through the
    Gaussian's chord by default.
    ...
    if chord == "auto":
        chord = "gaussian" if isinstance(beam, GaussianBeam) else "beam"
    if chord not in ("gaussian", "beam"):
        raise ValueError(f"joint_spectrum: chord must be 'auto', 'gaussian' or 'beam', got {chord!r}")
    ...
    w, v, u_b, flux_len, b_imp, z_ax = sample_atoms(rng, int(n_path), beam, T_C, half_window_m,
    ...
    env_rows = None
    if chord == "beam":
        u_along = getattr(beam, "u_fast", None) or beam.u
        env_rows = np.empty((w.shape[0], n_tau))
        for c0 in range(0, w.shape[0], 2048):              # chunked: 2048 x n_tau points a call
            sl = slice(c0, min(c0 + 2048, w.shape[0]))
            r = np.sqrt(b_imp[sl, None] ** 2 + (w[sl, None] * tau[None, :]) ** 2)
            env_rows[sl] = np.asarray(u_along(r, np.broadcast_to(z_ax[sl, None], r.shape)), float)
    ...
    for i in range(w.shape[0]):
        ...
        if env_rows is None:
            ...
            env = env_rows[i]
            phi = sign * 2.0 * math.pi * float(S0_mhz) * 1e6 * np.cumsum(env) * dt
            f = env * np.exp(1j * phi)
        Fa = np.fft.fft(f, n2)
...
class JointTable:
    ...
    def __init__(self, *, S0_grid: np.ndarray, w0_grid: np.ndarray, table: np.ndarray,
                ...
                z_ratio: float, half_window_m: float = float("nan"), isotope: int = 87):
        ...
        # ONE WAIST NODE IS A TABLE AT ONE WAIST (C6b, 2026-09-22): a fitter's Cell holds one waist, so its table
        # is read at that node exactly and interpolates in S0 alone, with no waist interpolation error at all
        if self.S0_grid.size < 2 or self.w0_grid.size < 1:
            raise ValueError("JointTable: S0_grid needs at least two points and w0_grid at least one")
        ...
        self.half_window_m = float(half_window_m)
    ...
    def build(cls, *, S0_grid: np.ndarray, w0_grid: np.ndarray, delta_mhz: np.ndarray,
             ...
             isotope: int = 87, half_window_m: Optional[float] = None) -> "JointTable":
        ...
        THE COLLECTED LENGTH IS THE OPTICS', NOT THE WAIST'S (C6b, 2026-09-22). A single `z_ratio`
        makes every waist node collect `z_ratio` of its OWN Rayleigh range, so across 41 to 56 um the
        collected half-length ran from 3.19 to 5.95 mm at z_ratio 0.6, where the bench's imaging fixes
        it at 3.375 mm and the fitter's own window reads it that way (`fullmodel.collection_z_ratio_m2`
        is that fixed length over each waist's Rayleigh range). `half_window_m` fixes the physical
        half-length for every node and is what a table the fitter reads must pass. `z_ratio` stays the
        default for the dimensionless studies built on it.
        """
        ...
        for j, w0v in enumerate(w0_grid):
            ...
            hw = float(half_window_m) if half_window_m is not None else collection_half_window_m(beam, z_ratio)
            for i, s0v in enumerate(S0_grid):
                table[i, j] = joint_spectrum(
                    S0_mhz=float(s0v), gamma_hom_mhz=0.0, beam=beam, T_C=T_C,
                    half_window_m=hw, n_path=n_path, seed=seed, delta_mhz=delta_mhz,
        ...
        return cls(S0_grid=S0_grid, w0_grid=w0_grid, table=table, delta_mhz=delta_mhz, m2=m2,
                  ...
                  half_window_m=(float("nan") if half_window_m is None else float(half_window_m)),
                  isotope=isotope)
    ...
    def _bilinear(self, s0q: float, w0q: float) -> np.ndarray:
        ...
        if n_w0 == 1:
            if abs(float(w0q) - float(w0g[0])) > 1e-9 * float(w0g[0]):
                raise ValueError(f"JointTable: a one-waist table holds {w0g[0]:.6e} m and was asked for {w0q:.6e} m")
            return (1 - ts) * table[i, 0] + ts * table[i + 1, 0]
    ...
    @classmethod
    def cached(cls, cache_dir, tag: str = "", **build_kw) -> "JointTable":
        """`build`, stored under `cache_dir` by a key over every input AND this module's own source, so a
        table is sampled once for all the workers and realisations that read it, and a change to the Monte
        Carlo invalidates every table built before it. The write is atomic (a temporary file renamed into
        place), so two workers racing to one key both end with a whole file. A `beam_factory` has no stable
        text to hash, so it must come with a `tag` naming the beam, and the tag is what the key reads."""
        import hashlib
        import json
        import os
        import tempfile
        if build_kw.get("beam_factory") is not None and not tag:
            raise ValueError("JointTable.cached: a beam_factory needs a tag naming the beam, since the factory "
                             "itself has no stable text to key the table on")
        h = hashlib.sha256(Path(__file__).read_bytes())
        h.update(("tag:" + tag).encode())
        for k in sorted(build_kw):
            v = build_kw[k]
            if k == "beam_factory":
                continue
            h.update(k.encode())
            h.update(np.asarray(v, dtype=float).tobytes() if isinstance(v, (np.ndarray, list, tuple))
                     else json.dumps(v, default=repr).encode())
        path = Path(cache_dir) / f"joint_table_{h.hexdigest()[:20]}.npz"
        if path.is_file():
            return cls.load(path)
        table = cls.build(**build_kw)
    ...
    def save(self, path) -> None:
        ...
        np.savez_compressed(Path(path), S0_grid=self.S0_grid, w0_grid=self.w0_grid,
                            ...
                            half_window_m=self.half_window_m, isotope=self.isotope)
    ...
    def load(cls, path) -> "JointTable":
        ...
        with np.load(Path(path)) as d:
            return cls(S0_grid=d["S0_grid"], w0_grid=d["w0_grid"], table=d["table"],
                      ...
                      half_window_m=float(d["half_window_m"]) if "half_window_m" in d.files else float("nan"),
                      isotope=int(d["isotope"]) if "isotope" in d.files else 87)
