...
fitted traces across 70–130 °C and 25–225 mW, three bounds, each 95% and each
conditional on the 64 µm waist prior: collisional self-broadening
β_self < 0.03-0.05 MHz per 10¹² cm⁻³ across the four peaks, laser width
σ_laser = 1.75–2.15 MHz across the four temperature blocks, and the AC-Stark
coefficient S₀(225 mW) < 0.15 MHz.
...
| AC-Stark coefficient **S₀(225 mW)** | < 0.15 MHz (95%, joint three-session profile likelihood with the threshold widened for block-to-block over-dispersion. Below the 0.35 predicted at the adopted waist, see [RESULTS](docs/RESULTS.md)) | bound | fixed lock + tighter focus |
