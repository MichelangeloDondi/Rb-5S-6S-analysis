...
| **w₀** | 64 µm (prior), not a measurement of this beam: the lineage was profiled on the previous laser, and this beam passes a 3 mm modulator aperture that one did not. A Gaussian fit of the line returns 42.0 ± 1.7 µm. **Closed on its own model the joint fit does not return an injected waist even at zero noise**, so the line constrains w₀ only up to a degeneracy with the refitted nuisances | carried, OPEN | a knife-edge scan here |
