...
are objects with real limits (`rb5s6s/instruments.py`), every number taken
from a manufacturer manual, and the two resolution mechanisms, disjoint
boxcar and moving average, are distinguished in code instead of treated
alike. The platform now decides the blackbody term: the heated cell
