...
This thesis reports a beam diameter of 128 µm, radius 64 µm, and **it is not this campaign's beam**. The sentence that stood here until 2026-09-16 called it a same-bench measurement taken on the same laser, and that is wrong twice over. [nieddu2019](nieddu2019.md) p. 6530 carries the identical 128 µm on the preceding laser generation, so these are one measurement reported twice and not two measurements, and the owner retired it as the waist authority on 2026-09-10 for that reason. He restated it on 2026-09-15: the waist is not measured here because that profiling used another laser source, and the 2025 beam additionally passes a 3 mm modulator aperture the profiled one did not. Both known differences push the effective waist above the transferred value. This value was carried into [`rb5s6s/constants.py`](../../rb5s6s/constants.py) as `W0_MEASURED_M`, a convention across apparatus whose name asserted what its own docstring denied. Owner order O44 (2026-09-21) retired it, and the renamed `W0_CENTRAL_M` now holds this record's own bore-limited focal radius (F280) instead of this thesis's transferred number, so no committed result stays conditional on this thesis's reading. What the campaign did not do is read the waist off its own beam at its own time, so drift or realignment since remains open and is the record's largest stated systematic, traced on [the beam waist](../wiki/the-beam-waist.md). The vapour-pressure route in `rb5s6s/density.py` and the cell apparatus described here both draw on Steck's *Rubidium 87 D Line Data*.
