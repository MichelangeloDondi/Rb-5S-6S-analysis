...
Held. The erratum's numbers are verified against the held PDF.

## What it corrects

> "The most important error was in the correction of our experimental
> data for the Zeeman effect."

The measurements were made in a magnetic trap at $B = 20.0$ G, in
$F=2, m=2$. The Zeeman adjustment was subtracted using a formula that
omitted the ground-state shift from $\omega'$. Correcting it:

> "Because of this mistake the reported value for $\lambda^{(0)}$ is
> incorrect. The correct value is $\lambda^{(0)} = 790.032326(32)$ nm, an
> adjustment of about $2\sigma$."
...
The theory correction is separate from the Zeeman one: the original
polarization averaging averaged $\lambda_0$ over two orthogonal linear
polarizations, valid "only if the derivative $d\alpha/d\lambda$ is
independent of the polarization angle, which is not the case here due to
the tensor contribution."

## The Lamporesi resolution

The 2015 paper noted "considerable disagreement" with Lamporesi et al.'s
790.018(2) nm without explanation. The erratum resolves it: Lamporesi
measured in the $F=1$ ground state, and Leonard's own measurement carried
to $F=1$ gives 790.017496(32) nm against a theoretical 790.0167(7), in
agreement. There is no outstanding experimental conflict on the Rb
tune-out.

## Use in this record

The computed tune-out (790.0339 nm) sits 1.57 pm from the corrected
value, against a 0.062 pm gap between the two published numbers, so the
correction changes no conclusion. The anchor quoted in
`rb5s6s/polarizability.py`, `docs/THEORY_NOTE.md`,
`scripts/run_polarizability.py` and the results ledger is the erratum
value, 790.032326(32) nm.
