...
**What the fit statistics say about sharing, and how much the verdict depends on the
...
correction does two things at once, and naming both is what makes the move
unsurprising: it rescales the chi-square as well as the sample count, so the freer
model's chi-square advantage shrinks roughly as the over-sampling factor while the
parameter penalty shrinks only logarithmically, and the verdict moves toward the
shared model under the correction, across zero.

A dataset whose verdict changes sign under the counting
