...
![Level diagram of the 993 nm two-photon cascade next to a hyperfine scan](../../figures/fig13_level_scheme.png)

*The 993 nm cascade drawn with its virtual intermediate level, alongside a
cavity-scan trace of the four resolved hyperfine components.*
...
why an S to S transition is drivable by two photons and not by one: both S
levels have the same, even parity, so a single photon cannot join them and
two of the same photon can. A three-photon transition flips parity back to
opposite, and the alternation continues with every photon added.
...
what makes the process two-photon and virtual, not a two-step,
...
decide which of them survive here. Angular momentum alone removes the
tensor piece, which has no reduced matrix element between two $J=1/2$
...
sits six orders below the tightest bound this record carries, a result
published for this exact transition. The scalar term, proportional to the
degree of linear polarisation squared, is what the two-photon rate reduces
to, and it vanishes identically for two photons of the same circular
handedness, since two vectors of equal helicity have a self dot product of
zero. [Rajasree 2020](../lit/rajasree2020spin.md) reports this law directly
on the $5S_{1/2} \to 6S_{1/2}$ transition in warm rubidium vapour, the rate
following the squared degree of linear polarisation and dropping to zero
for circular light.
...
photon obeys, so it can reach levels two photons cannot. The rate follows
the same construction one step further: three factors of the field in the
amplitude give a rate that scales as the cube of the intensity, not the
square. Each extra photon order also adds another off-resonant energy
denominator, so at a given available power the achievable rate falls
...
a different number of photons engages a different parity channel and a
different angular-momentum structure in the coupling. The tensor
decomposition turns polarisation into a lever on that structure directly.
An experiment can switch a whole tensor component of the coupling off at
the source by choosing a polarisation combination, instead of inferring
...
baseline in hardware instead of in a fitted baseline model.
[Doppler-free geometries](doppler-free-geometries.md) derives the same
cross term and same-beam term independently, from the
wavevector-cancellation condition instead of from polarisation. Circular
...
![Fringe amplitude versus fringe mean at two retro-reflection qualities](../../figures/fig25_retro_combination.png)

*Standing-wave fringe amplitude versus fringe mean at two retro-reflection
...
neither. A degenerate S to S two-photon transition keeps only the scalar
part of the coupling, and that term dies for two photons of the same
handedness, so circular light is a null test on the whole transition, not a
filter that separates the line from its pedestal the way orthogonal linear
polarisation does.

A polarisation optic set a few degrees off its nominal angle leaves a small
residual scalar term, not an exact zero, and if the residual signal sits
below the detection floor the trace looks identical to a true extinction.
An extinction test on its own bounds how close to circular the light was.
It does not certify that the light was exactly circular, and the bound is
only as tight as the noise floor the residual has to clear.

A common mistake is using the wrong dot product. The operator in the
two-photon amplitude is the plain, unconjugated
...
elliptical ones, so a null predicted from an instrument reading the
conjugated quantity, instead of from the polarisation state itself, can
land at the wrong angle entirely.

A real wave plate is not an ideal one. Its retardance is exact at one
design wavelength and drifts away from a quarter wave with wavelength and
temperature, and any birefringence elsewhere in the beam path, a window, a
lens, a mirror coating, adds its own small elliptical component on top.
[The fixed-lock instrument](../plan/10_the-fixed-lock-instrument.md)
records that this apparatus does not yet carry the insertable quarter-wave
plates the configuration needs, so for now the extinction null is a
hardware gap to close, not a floor already measured on this bench.

## Try it

The scalar two-photon coupling, $\vec\epsilon_1\cdot\vec\epsilon_2$ with no
conjugate, is evaluated for one fixed beam against a second beam whose
polarisation is set by a quarter-wave plate at a rotating angle. The rate
falls to its extinction null where the plate turns the fixed linear input
circular.
...
- G. Grynberg and B. Cagnac, "Doppler-free multiphotonic spectroscopy,"
  Reports on Progress in Physics 40, 791 (1977), the article that develops
