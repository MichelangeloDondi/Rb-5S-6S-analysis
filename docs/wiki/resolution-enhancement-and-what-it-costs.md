...
reported number. What is easy to get wrong is how much resolution is gained,
where the feature sits in the instrument, and what else it changes.

## Two rates of resolution gain

Averaging $N$ samples increases resolution at two different rates, and
instrument documentation quotes both, sometimes on the same page.

![Word length and noise resolution grow at different rates on a log-N plot](figures/wiki_resolution_enhancement_and_what_it_costs.png)

*Word length and noise resolution grow at different rates. Reading one for
the other doubles the claimed improvement.*
...
variance. The Agilent manual states both: an extra bit per factor of two, and
...
Quantisation is harmless whenever the step is small against the noise,
because the noise itself moves the signal across codes and averaging
recovers what rounding discarded. The quantisation contribution adds in
quadrature,
...
by 1.8 per cent. Neither instrument was resolution-limited as it was used,
and the four-bit difference between them gained nothing on the traces that
were taken. The bits are unspent headroom, and the place they would be spent
...
Where the feature is a math function it creates a separate trace, and saving
...
enabled, leave identical files.

The practical rule is to check the exported file, not the front panel. The
smallest nonzero voltage difference in the file is the quantisation step, and
$\log_2(\text{swing}/\text{step})$ is the delivered word length. It takes one
pass over the data and it cannot be fooled by the display.
...
ERes is specified that way, its manual stating that the filters do not
distort the relative position of different events and that the usual
filtering delay is exactly compensated. A plain causal average does move
features, by roughly half its window.
...
second is assuming the exported data carries what the screen showed. The
third is reaching for more bits when the dither ratio already says
quantisation contributes a fraction of a per cent, spending bandwidth for no
benefit.

A fourth is subtler and worth stating because smoothing invites it.
Averaging adjacent samples correlates them, so a smoothed record contains
fewer independent points than it has samples, and any uncertainty computed
as if the samples were independent is too small. That is the subject of
[correlated samples and effective sample size](correlated-samples-and-effective-sample-size.md),
and a smoothing setting is one of the things that can put the correlation
there. Where the smoothing decimates to the output rate the stored samples
can still be independent, and where it filters without decimating they are
