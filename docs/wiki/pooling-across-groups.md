...
A shared parameter asserts that one physical quantity produced every group, and
...
share the quantity, pooling is close to free: each group adds its own leverage,
and a group recorded over a wider range of the driving variable adds more than
a group recorded over a narrow one.

![Joint and separate fits of four repeats sharing one width](figures/wiki_joint_fit_toy.png)

*Four repeats that genuinely share one width, fit jointly and separately: the case where pooling costs almost nothing.*

Extending the lever should tighten the answer. If a group covers three times
the range and the interval widens instead, something in the setup is not what
the fit assumes.

## The sharing condition

Pooling adds information when the shared parameter means the same thing in
every group. It fails in two distinguishable ways.

- The groups differ in a way some free parameter absorbs, such as a different
  detector gain, offset, or noise scale. Giving each group its own nuisance
...
The common shape of the second failure is a parameter that scales with
something geometric or instrumental that was never recorded per group. The fit
...
Both diagnostics above are visible in that fit. The profile is scanned in both
...
squared-shift reach of the others, leaves the bound wider instead of narrower.

## Repeats and the root-n floor

For joint fits over repetitions of the same condition, does the uncertainty
fall as the root of n the way separate fits averaged together would? Only for
the noise that is independent between the repeats. The variance of a shared
parameter behaves as the condition-common part plus the per-repeat part over n,
so the common part sets a floor that no amount of joint fitting removes, and a
fit that models the repeats as independent will report root-n shrinkage
whether or not it is earned.

The split was measured for the collisional chain by restricted maximum
likelihood ([`beta_self_probe.csv`](../../results/beta_self_probe.csv), the
pooled rows): per-repeat scatter 0.133 MHz, condition-common 0.073 MHz, common
fraction 0.23. At those numbers five repeats give a factor of 1.6 instead of
...
independent part, and joint is only genuinely better where a single trace
barely constrains a nonlinear parameter.

The fix is to interleave and re-lock between repeats, and spread a condition's
repeats across the session, which converts common scatter into the kind that
...
- Absorbing a group difference with a nuisance fitted from the same channel as
  the answer. If a group's calibration comes from the same widths the answer
  is read from, the nuisance is not independent, and the two are coupled.
- Comparing a subset against the pool using a row that is not a subset fit.
  Reading one group's chi-square along the pooled profile gives a number that
  looks like that group's answer and is not, because the nuisances were fitted
  on everything. This record made exactly that mistake and did arithmetic on
  it.
- Treating a badly behaved likelihood as proof the groups disagree. A second
  ...
  differ needs a separate measurement.

## Try it

Two groups, each with its own free offset, one shared slope. The second group
covers three times the range of the first, so it carries much more leverage.
The only thing that changes between the two runs is whether its slope agrees.

![Bar chart of the shared-slope bound for three lever cases](figures/wiki_pooling_across_groups.png)

*The bound from the Try it calculation: an agreeing long lever tightens it, a disagreeing one widens it instead.*
...
The agreeing group tightens the bound by about a quarter. The disagreeing
group carries identical statistical weight, and adding it widens the bound
instead of narrowing it, without the fit flagging anything. The only tell
available without already knowing the truth is that the longer lever failed
to tighten the interval.

## Further reading

The general treatment of hierarchical and partially pooled models is standard
in the statistics literature under random effects and multilevel models, where
the question of this page appears as whether a coefficient should be fixed
across groups or allowed to vary. The vocabulary differs and the decision is
the same one.
...
- [Identifiability](identifiability.md), for the case where the data do not
  determine the parameter at all, which pooling improves the precision of
  without resolving.
- [The profile likelihood](profile-likelihood.md), for the interval
  construction the demonstration above uses, and for why both scan directions
  are run.
- [When a joint fit is legitimate](../big_picture/08_when-a-joint-fit-is-legitimate.md),
