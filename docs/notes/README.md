...
| [two_photon_saturation_companion.md](two_photon_saturation_companion.md) | The two-photon Rabi frequency at the campaign's brightest field, computed because a question about a third photon needed it, and the consequence that atomic saturation broadens the line about 3.7 times harder than the AC-Stark ramp the width-channel light-shift lever is built on, with the same power signature and absent from that forward model. The direction of the bias is favourable, so no committed bound changes, and the note states what would settle it. A dated addition carries a second companion of the same signature, hyperfine pumping by the real cascade, whose width is exactly the branching fraction times the saturation width, and a second addition records why the joint three-session bound was not re-profiled. | open |
