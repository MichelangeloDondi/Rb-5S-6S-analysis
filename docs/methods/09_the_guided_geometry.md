...
## 9. The guided geometry in an evanescent field

This chapter asks what each term of the cell's lineshape becomes for an atom in the evanescent field of an optical nanofibre, and which of them changes form and not merely size? This chapter builds on the lineshape and AC-Stark chapters, whose four terms it carries across, and a solved HE11 mode from `rb5s6s.fibre` and sets out the guided transit kernel and its second-order entry into the width, the light shift referred to the field an atom actually sees, and the atom-surface term the cell has no analogue for. Not covered here: you have no interest in guided geometries. Nothing in the vapour-cell result depends on this chapter, and the fibre thread is declared in [BIG_PICTURE](../BIG_PICTURE.md) so it can be skipped whole.
...
committed table is reproduced only by the standard-deviation reading.
...
total width is correspondingly weaker.
...
### 9.6 Twin forecast of gain and cost
...
### 9.7 Interaction length and the transmission observable
