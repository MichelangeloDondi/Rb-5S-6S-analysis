...
> (with lettered fitting sub-stages M4b–M4e), **802 tests**
...
tests/    802-test battery (~778 fast ~90 s + 25 `slow` high-statistics
...
pip install -e ".[dev]" && pytest -q          # 778 fast tests (~90 s)
pytest -q --runslow                           # full 802 incl. slow closures (what CI runs)
