...
> (with lettered fitting sub-stages M4b–M4e), **1203 tests**
...
tests/    1203-test battery (1172 fast ~2 min + 31 `slow` high-statistics
...
pip install -e ".[dev]" && pytest -q          # 1172 fast tests (~2 min)
pytest -q --runslow                           # full 1203 incl. slow closures (what CI runs)
