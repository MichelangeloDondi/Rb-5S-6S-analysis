...
> (with lettered fitting sub-stages M4b–M4e), **800 tests**
...
tests/    800-test battery (~776 fast ~90 s + 25 `slow` high-statistics
...
pip install -e ".[dev]" && pytest -q          # 776 fast tests (~90 s)
pytest -q --runslow                           # full 800 incl. slow closures (what CI runs)
