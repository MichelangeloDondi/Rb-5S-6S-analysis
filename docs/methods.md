...
> (with lettered fitting sub-stages M4b–M4e), **975 tests**
...
tests/    975-test battery (945 fast ~2 min + 30 `slow` high-statistics
...
pip install -e ".[dev]" && pytest -q          # 945 fast tests (~2 min)
pytest -q --runslow                           # full 975 incl. slow closures (what CI runs)
