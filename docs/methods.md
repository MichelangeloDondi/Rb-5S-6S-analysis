...
> (with lettered fitting sub-stages M4b–M4e), **1205 tests**
...
tests/    1205-test battery (1174 fast ~2 min + 31 `slow` high-statistics
...
pip install -e ".[dev]" && pytest -q          # 1174 fast tests (~2 min)
pytest -q --runslow                           # full 1205 incl. slow closures (what CI runs)
