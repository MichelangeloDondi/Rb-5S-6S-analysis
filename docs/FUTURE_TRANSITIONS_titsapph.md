# Future transitions on the tunable Ti:Sapph
...
The drive laser tunes, so what else could this bench measure, and what would each option be worth? This page builds on [BIG_PICTURE.md](BIG_PICTURE.md) §1, for why this class of line is worth the work at all and sets out the candidate rubidium two-photon lines ranked, the computed magic wavelengths, the one-colour three-photon rung and the scaling argument that makes it the cliff regime rather than the delicate measurement it was proposed as. Not covered here: the 2025 result. Nothing in this document is a measurement, and several of its options are here because working them out was the only way to find out they do not work.
...
crossing, so the bracket is what to read: 1.67 nm at the 1204 nm crossing <!-- other-quantity: the magic-wavelength Monte-Carlo bracket, not the 7S Rayleigh-range error -->
...
## 2. The transition menu the Ti:Sapph opens
...
|---|---|---|---|---|---|---|
| **5D₅/₂** | **778.10 nm** | 1.1 THz | **near-resonant** (5P₃/₂) | 420 nm (6P→5S) | 725–875 / 725–975 / **700–1000** | **hot** (clock frontier) |
| **7S₁/₂** | **760.13 nm** | 10.2 THz | intermediate | 420 nm (6P→5S) | 725–875 / 725–975 / **700–1000** | light (Morzyński, Chui) |
| 6D_J | 697.45 nm | 45.6 THz | ~clean | 420 nm (6P→5S) | **670–710 only** (blue set) | none |
| 8S₁/₂ | 688.56 nm | 51.2 THz | ~clean | 420 nm (6P→5S) | **670–710 only** (blue set) | none |
| 7D_J | 660.77 nm | 69.5 THz | clean | 420 nm (6P→5S) | custom <700 nm | none |
| 9S₁/₂ | 655.78 nm | 72.9 THz | clean | 420 nm (6P→5S) | custom <700 nm | none |
| **6S₁/₂** | **993.42 nm** | 75.3 THz | **clean** | 795 nm (5P₁/₂→5S) | **700–1000** (edge) or 950–1050 ← *current* | none (us + USAFA) |
| 4D_J | **1033.30 nm** | 86.9 THz | clean | 1.48/1.53 µm (4D→5P), then 795 | 950–1050 / custom >1000 | 4D_J clock (2024) |


Two rows of this table were wrong until 2026-09-09 and two rungs were
missing. It read "8S₁/₂ at 697 nm" and "9S₁/₂ at 660 nm". Those wavelengths,
and the intermediate detunings and thermal gaps computed from them, belong to
**6D and 7D**. The true 8S sits at 688.56 nm and the true 9S at 655.78 nm.
Every column of the two rows was internally consistent, which is why it read as
verified: they were consistently the D state's. The ladder that settles it is
built from this package's own held levels and lands in
[`results/transition_ladder.csv`](../results/transition_ladder.csv). Each
extrapolated **S** rung carries plus or minus about 0.045 nm, which is the Ritz
form's own error measured by predicting nP states it was not fitted to, against
a discrepancy of 8.4 nm.

The extrapolated D rungs carry 0.35 nm, seven
times that, and the number is not the nP analogue but the miss against an
independent route: the held 5P₃/₂ level plus the 5P₃/₂→nD line puts 6D at
697.12 nm against this ladder's 697.45. Those two line wavelengths are not held
here, so both D rungs are owed a NIST lookup and neither is quotable to better
than a few tenths of a nanometre. **The D rungs are the weaker half**: only two D anchors
are held, so their form error cannot be tested by leaving one out, and an
independent value for 6D sits 0.35 nm from this ladder. The S rungs are settled
and the D wavelengths above are the ladder's, owed to a NIST lookup. Register
A139 carries the working. **The energies here should still be
replaced by NIST values before any of them is quoted as a measurement**, which
is a literature lookup and not a bench question.


### Literature coverage of these rungs

*Added 2026-09-10.* The polarizability column of this menu was empty for five
rungs, and the values were in a paper this repository already held. Safronova
and Safronova, Phys. Rev. A 83, 052508 (2011), its sixth table carries the scalar
static polarizability of every rung here except 7D, with its own uncertainties,
and its second carries the reduced E1 matrix elements behind them. The reading was
cross-checked against this record's own sums where both exist, agreeing to
0.04 per cent on 6S and 0.7 on 7S.

| rung | drive (nm) | static alpha(upper), a.u. | differential at the drive |
|---|---|---|---|
| 4D | 1033.30 | 541(26) | owed |
| 6S | 993.42 | 5169(21) | [-1131.80](../results/transition_ladder.csv "ref:transition_ladder:6S:differential_polarizability") |
| 5D | 778.10 | 17500(150) | [28648.70](../results/transition_ladder.csv "ref:transition_ladder:5D:differential_polarizability") |
| 7S | 760.13 | 32630(140) | 4371.66 <!-- other-quantity: the last digits' uncertainty on a term energy, not the dilute-gas margin of docs/methods/02 --> |
| 6D | 697.45 | 91580(620) | owed |
| 8S | 688.56 | 133200(650) | owed |
| 7D | 660.77 | not in the table | owed |
| 9S | 655.78 | 417200(2200) | owed |

The table is static and the campaign's differential is not. Equation 8 of
that paper carries no frequency. At 688.6 nm the 8S to 5P denominator is
-16467 cm-1 against a photon of 14523, so the dynamic value is nowhere near the
static one. What the static column does is rank the rungs and close a dynamic
sum at zero frequency. The differential column stays owed, and its route is
that paper's matrix elements summed on NIST energies. Register A143 carries the working,
including a Coulomb-approximation build this retired.

### The thermal field as a selection criterion
...
|---|---|---|---|---|---|
| 4D_J | 1033.3 nm | 2.30 µm | 15.5 | 1.8e−7 | negligible |
| **6S₁/₂** | **993 nm** | **2.79 µm** | **12.8** | **2.8e−6** | **negligible** |
| 5D₅/₂ | 778 nm | 5.23 µm | 6.8 | 1.1e−3 | watch |
| 7S₁/₂ | 760 nm | 6.56 µm | 5.4 | 4.4e−3 | watch |
| 6D_J | 697 nm | 12.42 µm | 2.9 | 6.0e−2 | **matters** |
| 8S₁/₂ | 689 nm | [12.68](../results/transition_ladder.csv "ref:transition_ladder:8S:nearest_np_gap") µm | 2.8 | 6.4e−2 | **matters** |
| 7D_J | 661 nm | 24.19 µm | 1.5 | 3.0e−1 | **matters** |
| 9S₁/₂ | 656 nm | [21.69](../results/transition_ladder.csv "ref:transition_ladder:9S:nearest_np_gap") µm | 1.6 | 2.4e−1 | **matters** |

Upper-state energies and the nP ladder both come from
[`results/transition_ladder.csv`](../results/transition_ladder.csv) now, which
is why two rows moved on 2026-09-09: this table declared it took its energies
from the menu above, and it did, including the two the menu had mislabelled. A quarter of a photon per mode at 9S
...
is **950–1050 nm**, 993 and 4D (1033.3) are easy but 5D/7S require an optics-set swap.
8S (688.6) needs the dedicated 670–710 blue set. 6D (697.5) sits in that same
set, while 7D (660.8), 9S (655.8) and any >1000 work are custom.

### The laser: M-Squared SolsTiS and Coherent Verdi V18
...
and, on the same cascade, the 6D and 7D rungs the corrected table adds, an
assignment made here from the level structure and not yet from a held source,
...
### Detection gain from the 795 nm filters
...
- **Corrected 2026-08-03 (see §3.2): the requirement above is not hard for 7S,
  ...
  *et al.* 2025 likewise read 5D₃/₂ on 795, 762 and 420 nm
  ...
  drive.
- The payoff is broad: **one 420 nm path serves 5D, 6D, 7S, 7D, 8S and 9S together** (all
...
But "cheap" describes the hardware and not what could break the session.
What could break it is the noise floor, which has to be measured rather than
...
and the third moment, against values another group has already measured to
...
follow. The single-colour drive at **[778.104](../results/transition_ladder.csv "ref:transition_ladder:5D:drive_wavelength") nm** (computed in §3.4) is pinned
by the resonance and cannot be tuned, so the field scanned across 776.179 nm is
the *perturbing* one, not the drive.

And that is exactly the configuration in
...
### 3.2 Closure of the anchor loop by 7S
...
(2026-08-05):
    ...
    beta_self(6S) = beta_self(7S)_measured * [DC6(5S+6S) / DC6(5S+7S)]^(2/5)
                  = 5.624 * 0.3168^0.4 * 0.985
                  = 3.50 +- 0.37 kHz per 1e12 cm^-3

    DC6(5S+nS)    = C6(5S+nS) - C6(5S+5S)

with C₆(5S+6S) = [54008](../results/beta_self_theory.csv "ref:beta_self_theory:pair_coefficients:c6_5s_6s") a.u., C₆(5S+7S) = 161474 a.u. and C₆(5S+5S) = 4180 a.u.
...
states and cancel in it. The module predicts
    ...
    beta(6S) / beta(7S) = 0.3168^0.4 * 0.985 = 0.622
...
### 3.3 The ladder above its rungs
...
C₆(5S+nS) growing as **n\*^3.6** ([54008](../results/beta_self_theory.csv "ref:beta_self_theory:pair_coefficients:c6_5s_6s") to 161474 a.u., with n\* = 2.845 and
...
faster because the same 4180 a.u. is subtracted from both, **n\*^3.8** (49828
to 157294 a.u.), so β_self would grow as **n\*^1.5** across the same step. Two points give a local slope.
...
|---|---|---|---|
| 5S→6S | [993.418](../results/transition_ladder.csv "ref:transition_ladder:6S:drive_wavelength") nm | [-1131.8](../results/polarizability_deep.csv "ref:polarizability_deep:delta_alpha:at_drive") ± [5.9](../results/polarizability_deep.csv "ref:polarizability_deep:delta_alpha:at_drive:err") dynamic | sign under dispute, THEORY_NOTE §5; the dynamic tail in `results/polarizability_deep.csv` |
| 5S→7S | [760.126](../results/transition_ladder.csv "ref:transition_ladder:7S:drive_wavelength") nm | +4372 a.u. | independent line list, Safronova 2004 |
| 5S→5D₅/₂ | [778.104](../results/transition_ladder.csv "ref:transition_ladder:5D:drive_wavelength") nm | ≈ +28600 a.u., anchored not recomputed | J = 5/2, tensor term dropped, magnitude only |
...
Ti:Sapph ladder block header), and Hamilton's measurement is accepted instead.
...
what the accepted measurement licenses. Hamilton's measured 776.179 nm magic
wavelength is where Δα crosses zero. Moving from there to the [778.104](../results/transition_ladder.csv "ref:transition_ladder:5D:drive_wavelength") nm drive,
...
with the same input beam reaches `w0 = λf/(πw_in)`, so the 42.38 µm convention at
993.4 nm becomes [32.18](../results/projections.csv "ref:projections:input_rung_waist_at_drive:760 nm, 5S to 7S") µm at 760.1 nm, and the
...
|---|---|---|
| 5S–6S | [1203.7](../results/polarizability.csv "ref:polarizability:magic_5s6s:1204nm") nm (α = +547 a.u.), [1287.9](../results/polarizability.csv "ref:polarizability:magic_5s6s:1288nm"), 1339.6 nm | 1204 is far from every 6S resonance and traps both states, the other two sit among the 6S–5P and 6S–7P lines |
| 5S–6S, inside the Ti:Sapph band | 790.1 nm (α = −244 a.u.), **not quotable** | between the 5S D lines, 0.1 nm from the ground-state tune-out, so a ground-state vapour absorbs it hard. **And the 6S sum is truncated at 8P, whose line lies at 1028.7 nm, so this crossing sits between the omitted 13P near 797.9 and 14P near 787.1 nm <!-- other-quantity: the 14P line's wavelength in nm, not a projections factor --> and is computed without the states that dominate there.** `magic_wavelengths` is scoped to 950 nm and above for this reason |
...
| 5S–5D₅/₂ | 776.179(5) nm, measured | 0.2 nm from the 5P₃/₂–5D₅/₂ resonance, also an excited-state transition, and 1.9 nm from the [778.104](../results/transition_ladder.csv "ref:transition_ladder:5D:drive_wavelength") nm two-photon wavelength |
...
-102, 9P and 10P +11.9 and the tail and core +10.6 (`rb5s6s.polarizability`, term by term), and
the sign dispute in the literature is a dispute about that balance. The crossing
near 1204 nm is set by the same balance and is correspondingly sensitive: scaling
both 6S to 6P elements by 0.95 moves it to 1192.5 nm and by 1.05 to 1214.2, about  <!-- other-quantity: a multiplicative scaling on a matrix element -->
...
the upper state's polarizability less the ground state's.

Because a null needs no
...
the observable. At 225 mW and 42.38 microns the probe's own shift is 725 Hz per
...
[760.126](../results/transition_ladder.csv "ref:transition_ladder:7S:drive_wavelength") nm.
...
### 3.4 A doubling stage, as an options map
...
- **Not a collinear Doppler-free three-photon route either, although it looks
  ...
  **1187.3 nm**. The reddest catalogue set stops at 1050 nm, where the sum
  overshoots by 4404 cm⁻¹, and the present [993.418](../results/transition_ladder.csv "ref:transition_ladder:6S:drive_wavelength") nm drive overshoots by
...
- **5S→5P₁/₂ at 795.0 nm plus 5P₁/₂→6S at 1323.9 nm, or 5S→5P₃/₂ at 780.2 nm
  ...
  the [1339.6](../results/polarizability.csv "ref:polarizability:magic_5s6s:1340nm") nm crossing of §3.3.
...
is $f(s)\propto s^{n-1}$ on $[0,S_0]$ with $n$ the number of photons, so
$n=2$ gives the triangle the 993 nm work fits and $n=3$ gives a parabola,
$f(s)=3s^2/S_0^3$. Its moments follow by the same direct integration:

$$\langle s\rangle=+\tfrac34 S_0,\qquad \mathrm{Var}=\tfrac{3}{80}S_0^2,\qquad
\mu_3=-\tfrac{1}{160}S_0^3,\qquad g_1=-\tfrac{2\sqrt{15}}{9}=-0.861$$

so the intrinsic standardised skew is **1.52 times the triangle's 0.566 in magnitude**, on a
...
larger: 5P3/2-4d5/2 is the committed 10.90 a.u. against 6S-8p3/2 at 0.629, and <!-- other-quantity: a matrix element -->
...
fix is the STAR: three coplanar beams of equal $|k|$ at 120 degrees, whose wave <!-- other-quantity: an angle in degrees -->
...
### Addendum, 2026-08-09: four omissions
...
take the 21 to about 2.6.

Whether it inverts needs the full recoupling
...
detection and fitting problem rather than a rate problem.
...
at the cell's 400 K needs the 120 degree vertices held to roughly 24 arcseconds <!-- other-quantity: an angle in degrees -->
...
systematic, and it reaches the assumed retro ratio in about 2.1 hours. Both
...
and reaches the assumed retro ratio in about 2.1 hours, both on the comb of four
...
plus one 130 °C point.

If that bridge did not hold, nothing would be
retracted, because the statements already published are conditional
on the waist convention.
...
along with the first fixed-lock laser width.

stage 2 buys handle strength
...
signal to about 3 to 12σ, so both halves have to work. The third moment
...
Even then the geometry blocks would still convert the
...
13 sigma, against 4 sigma with the scatter uncut, which is the
...
A rate measured here, with the convention stated, would
replace the choice between them, and a measured β(6S)/β(7S) would test the
rate ratio the module predicts at 0.622, the ΔC₆ ratio 0.3168 to the
...
which is the same open question as at 7S.

And the magic-wavelength scan is a
scan of a perturbing beam rather than of the drive, since a single-colour
drive is pinned at [778.104](../results/transition_ladder.csv "ref:transition_ladder:5D:drive_wavelength") nm, so the closed-form shift distribution holds
...
allows. The magic-wavelength scan is sized the same way.
...
each point good to 8 percent of the shift at the edge of that span.
...
at [1287.9](../results/polarizability.csv "ref:polarizability:magic_5s6s:1288nm") nm would push toward.
...
and Wall do not perform, and the third moment as a drift-immune channel.
...
pinned at [778.104](../results/transition_ladder.csv "ref:transition_ladder:5D:drive_wavelength") nm and the field scanned across the crossing is the perturbing
...
### 5.1 The steep root at 1297.5 nm
...
campaign intensity and the waist convention, so these are envelope numbers in
...
calibration anywhere in the chain.

And the doublet offers a second
handle: a companion null at [1287.88](../results/polarizability.csv "ref:polarizability:magic_5s6s:1288nm") nm sits 4.5 nm below the 7P 3/2 pole
...
at 1502 nm, and on the ground state everything sits below 520 nm. So the <!-- other-quantity: a wavelength -->
neglected terms contribute background, never a local pole.
...
#### 5.1.1 Amendment, 2026-08-08: the other five roots
...
176 atomic units for a one per cent change at [1339.6](../results/polarizability.csv "ref:polarizability:magic_5s6s:1340nm") nm against 163 at the
steep root.
...
| [1203.7](../results/polarizability.csv "ref:polarizability:magic_5s6s:1204nm") | +0.012 | 23400 | 6S–5P₃/₂ | 11% | 0.21% | 0.02 |
...
rather than a materially tighter number. The apparent better lever at [1339.6](../results/polarizability.csv "ref:polarizability:magic_5s6s:1340nm") nm
...
shift precision to wavelength rides the campaign intensity and the accepted
...
The fixed-lock session shot-list temperature range would differ per transition.
- The ruler comb itself: in the 2025 dataset the scan clips one third-order
  tooth window on every recorded trace, and at the measured drive depth
  (2β = 1.57 median across the combs) a fully covered
...
947.3. From the upper level it enters at n = 11 and runs to 958.1.
