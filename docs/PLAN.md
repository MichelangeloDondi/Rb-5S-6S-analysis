...
| 5 | No acquisition clock available to the analysis | block order the only clock — and **not even the acquisition order** (the power ladder ran 225→25 mW) | σ_laser-sharing assumption untestable **as analysed**. The recovered backup (audited 2026-07-23; integrity void at T1, labelled post-hoc pass, clock committed as `data_recovered/CLOCK.csv`) has since **dated** it: the four peak-blocks of a dwell sit **54–76 min apart**, so "acquired close in time" was never true; their widths track that hour no better than chance (r = +0.18, p = 0.6, n = 12). Untested by the *design*, not by the missing log | interleave the four peaks within minutes + log scope clock (§8.4) |
