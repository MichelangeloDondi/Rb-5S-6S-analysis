...
**Assumption A1 (to confirm, one word)**: scope triggered on the sweep sync, so file time = ramp phase. A ramp-monitor channel demonstrably existed — a 2025-06-10 scope photograph shows the triangular sweep on one channel and the fluorescence on another, peaks mirrored about the apex ([APPARATUS.md](APPARATUS.md) §4.1) — so if that channel was saved for any archival trace, A1 is directly testable rather than assumed. Evidence: frozen repeat positions; window (1.000 s) ≈ one up-ramp (~43 MHz laser at measured rate, consistent with an earlier "~45 MHz" estimate). If false, Module M2 degrades gracefully to per-trace calibration.
