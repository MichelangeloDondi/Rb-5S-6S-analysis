# Paper portfolio — what we can write under each campaign scenario

**Status: planning / decision aid (2026-07-13).** Refines `PAPER1_SKELETON.md` §0.
The publishable output is contingent on two *independent* future campaigns; this
doc maps papers → scenarios so the writing plan degrades gracefully if one (or both)
slips. Honest throughout: the archive delivers **bounds + a validated method**, not
coefficients (see `PAPER1_SKELETON.md` §I) — the measurements need the campaigns.
...
2. **October cell fixed-lock top-up** (`PLAN.md` §8; the group's task #1) — knife-edge
   w₀, a *stable* lock (line centres come back), same-session high-T (150–170 °C) and
   ...
   420 nm detection allow.
3. **ONF measurements** (Zohreh; the Gokhroo/Le Kien/Nic Chormaic pushing-dip + ONF
   lineshapes — `gokhroo2022`). A separate campaign feeding Paper 2.

**Timeline anchor:** thesis due Oct 2026; the OIST visit is *post-submission*. So the
**thesis rides on the archive** (P1-min content + the campaigns as future work — this
is already done); the **papers** are what the campaigns unlock afterwards.
...
| **P1-full** | same, + the fixed-lock coefficients | archive + **October** | as P1-min, plus **measured** AC-Stark coeff (pull resurrected), β_self (high-T lever + knife-edge w₀), σ_laser | flagship; medium risk (October must deliver) |
| **P2** | *ONF pushing-dip / near-surface two-photon lineshapes* | **ONF** (+ cell as reference) | quantitative completion of Gokhroo 2022; surface/pushing effects vs the clean cell line | Zohreh collab; impact set by the ONF result |
...
## The 2×2 (October × ONF), Ti:Sapph 5D/7S as a conditional add-on

| | **no ONF** | **with ONF** |
|---|---|---|
| **no October** | **P1-min** only (1 paper: method + bounds). The safe floor. | **P1-min + P2** (2 papers; P2 uses the *archival* cell as reference — weaker but workable). |
| **with October** | **P1-full** (the flagship). **+ PA/PB/PC** iff the session tunes to 5D/7S (→ up to 4 papers). | **P1-full + P2** (P2 now uses the *October-refined* cell reference — the strongest Paper 2). **+ PA/PB/PC** if extended (→ up to 5). |

## Decision guidance

- **P1 is the anchor and it is not really optional** — the only choice is *when/what
  form*:
  - **(a) Hold for P1-full** (one combined paper, October fills §VII). Higher impact;
  ...
  - **(b) Publish P1-min now**, follow with a measurement paper after October. De-risks
    priority, gives an earlier output and a clean thesis chapter, at some cost of a
    thinner first paper. Choose this only if a publication is needed before October or
    a scoop worry emerges.
- **P2 is Zohreh's collaboration** and is gated on ONF, not on us — but it is *better
  with* October (a refined cell reference). If ONF happens before October, P2 can
  still proceed on the archival reference.
...
1. **Now → thesis:** write the thesis on the archive = **P1-min content** + the
   October/ONF/Ti:Sapph plans as future work. (The analysis is done; this is a
   writing task, not a measurement one.)
2. **Default first paper = P1-full**, submitted after a successful October cell
   session. Keep P1-min as the fallback if October slips past a needed deadline.
...
*Cross-refs: `PAPER1_SKELETON.md` (P1 structure), `PLAN.md` §8–9 (October + Ti:Sapph
