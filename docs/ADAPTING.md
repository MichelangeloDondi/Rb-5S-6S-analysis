...
| the shift's odd-moment channel | [`rb5s6s/cumulants.py`](../rb5s6s/cumulants.py) | the windowed self-centred cumulant of any order, the window recentred to a tolerance with the pass count and a converged flag returned, the detector pedestal removed from the trace's own wings first | a fixed pass count on a dim line reads a fake third moment that grows as the window cubed, and a pedestal left in the normalisation bends a power law | [`tests/test_cumulants.py`](../tests/test_cumulants.py), whose pedestal test carries the retired twenty-pass form as its negative case |
