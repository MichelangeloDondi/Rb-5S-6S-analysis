...
| the shift's odd-moment channel | [`rb5s6s/moments.py`](../rb5s6s/moments.py) | the windowed self-centred cumulant of any order, the window recentred to a tolerance with the pass count and a converged flag returned, the detector pedestal removed from the trace's own wings first | a fixed pass count on a dim line reads a fake third moment that grows as the window cubed, and a pedestal left in the normalisation bends a power law | [`tests/test_moments.py`](../tests/test_moments.py), whose pedestal test carries the retired twenty-pass form as its negative case |
| the full forward model and its fitter | [`rb5s6s/fullmodel.py`](../rb5s6s/fullmodel.py) | the composite line with the Doppler pedestal, the retro tilt's residual width, the saturation companion as its own parameter, the collection ratio carrying the beam quality, and a fitter that refuses any term the model cannot recover | six terms the world generates cannot be fitted at all and are listed with their reasons, so a closed loop that recovers what it injected is read as the tautology it is; `m2` is discontinuous at exactly 1 and the docstring says so | [`tests/test_fullmodel.py`](../tests/test_fullmodel.py) |
| where the atoms are, and what the trap does to them | [`rb5s6s/platforms.py`](../rb5s6s/platforms.py) | each platform's density, temperature and geometry, with `trap_depth_uk` and `trap_eta` owning the intensity convention | the two standing-wave factors are refused together, because applying both counts the retro twice and doubled a published trap depth | [`tests/test_platforms.py`](../tests/test_platforms.py) |
...
| the apparatus | [`rb5s6s/config.py`](../rb5s6s/config.py) for file layouts, directory roots, fit windows and QC thresholds. [`rb5s6s/constants.py`](../rb5s6s/constants.py) for the beam itself, since `W0_CENTRAL_M` (the calculated, bore-limited focus), `W0_BAND_M`, `RHO_RETRO` and `RHO_RETRO_ERR` carry provenance tags and live there. `config.py` re-exports those four by name, so a script importing them from either module gets the same value, and only `constants.py` is the place to change one | the peak light shift quoted at the wrong intensity, since it goes as the inverse square of the waist. **And the waist is not a bench constant across drive wavelengths**: `waist_at_drive` carries it through the fixed lens as $w_0 = \lambda f/(\pi w_{\rm in})$, so anyone adapting this record to another transition overrides `W0_CENTRAL_M` at their peril and calls that function instead. It takes the input-beam regime as a required keyword, aperture-limited or resonator-mode, because this record does not pin which the bench is in and the two differ by 31 per cent in the shift | [`tests/test_ramp_geometry_docs.py`](../tests/test_ramp_geometry_docs.py), which recomputes the geometry tables the documents print, and [`tests/test_drive_waist.py`](../tests/test_drive_waist.py), which refuses an unnamed regime |
...
from rb5s6s import (
    ...
    two_photon_rabi_hz,
    # the atomic inputs, continued: the 5D rung's own differential
    delta_alpha_5d,
    # the constants a caller most often needs to override
    W0_CENTRAL_M, RHO_RETRO, GAMMA_NAT_HZ, TAU_6S_S, LAMBDA_LASER_M,
    DELTA_ALPHA_AU, transit_fwhm_from_w0, waist_at_drive, __version__,
)
```
...
`width_identifiability` or read `corr_laser_coll` alongside every forecast: at
the twin's reference design the two widths correlate at about [-0.94](../results/twin_span_sweep.csv "ref:twin_span_sweep:span_060MHz:corr_laser_coll"),
and that number does not move materially with span or with repeat count. More data fixes noise and never fixes
